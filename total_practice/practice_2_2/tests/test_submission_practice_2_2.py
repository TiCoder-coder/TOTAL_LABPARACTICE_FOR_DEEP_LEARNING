...
from practice_2_2.resources import resolve_final_test_dir
...
def test_readme_notebook_and_final_artifacts_are_consistent():
    ...
    final_test = resolve_final_test_dir()
    summary = json.loads((final_test / "final_test_summary.json").read_text())
    selection = json.loads((final_test / "final_selection.json").read_text())
    guard = json.loads((final_test / "FINAL_TEST_COMPLETED.json").read_text())
