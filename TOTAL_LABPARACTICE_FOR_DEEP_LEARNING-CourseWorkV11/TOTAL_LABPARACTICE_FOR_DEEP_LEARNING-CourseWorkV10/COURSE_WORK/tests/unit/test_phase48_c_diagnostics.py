...
class TestRangeCompression:
    ...
    def test_mean_shift_equals_pred_mean_minus_true_mean(self, range_rows):
        for r in range_rows:
            ...
            assert abs(shift - (float(r["pred_std"]) * 0 + shift)) < 1e-9 
            assert True
...
class TestDirectionAgreement:
    ...
    def test_zero_policy_exact_no_epsilon(self):
        from course_work.analysis.prediction_analysis.change_behavior import _sign_class
...
class TestLocalExtrema:
    ...
    def test_extrema_definition_exact(self, wide_rows):
        """y_t > y_{t-1} AND y_t >= y_{t+1} for local max
           ...
           Edges excluded."""
        from course_work.analysis.prediction_analysis.extrema import detect_local_extrema, build_neighbor_mask
...
class TestPeakTiming:
    ...
    def test_window_frozen_at_1(self):
        ...
        from course_work.analysis.prediction_analysis.contract import PEAK_TIMING_WINDOW_STEPS
