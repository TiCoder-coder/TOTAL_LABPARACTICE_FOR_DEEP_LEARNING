...
import scripts.phase45_final_model_lock as lock_mod 
...
def _install_monkey_patches() -> None:
    ...
    torch.optim.Optimizer.step = _raise_step  

    try:
        from course_work.training import engine as _engine_mod  
        if hasattr(_engine_mod, "TrainingEngine"):
            def _train_guard(self, *args, **kwargs): 
                raise RuntimeError(
                    "REHEARSAL FORBIDDEN: TrainingEngine.train called in Phase45 no-train rehearsal"
                )
            _engine_mod.TrainingEngine.train = _train_guard  
        if hasattr(_engine_mod, "RefitEngine"):
            ...
            _engine_mod.RefitEngine.refit = _refit_guard  
...
def _restore(_orig_step) -> None:
    import torch
    torch.optim.Optimizer.step = _orig_step  
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase45_rehearsal_") as tmp:
        ...
        try:
            ...
            lock_mod.ARTIFACT_DIR = saved_dir 
