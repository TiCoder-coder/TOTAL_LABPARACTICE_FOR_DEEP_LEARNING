...
# ---- Invariant 2: LSTM handoff schema matches official schema ----
...
def test_invariant_2_lstm_handoff_schema_matches_official_schema():
    ...
    # Verify the disposable rehearsal consumes this canonical handoff
    with tempfile.TemporaryDirectory() as tmp:
        ctx = _make_ctx(Path(tmp))
...
# ---- Invariants 3/4/5/6/7: artifact counts ----
...
# ---- Invariant 8: 12/12 epoch propagation matches ----

def test_invariant_8_stage_b_epoch_propagation_12_of_12():
    with tempfile.TemporaryDirectory() as tmp:
        ctx = _make_ctx(Path(tmp))
        result = run_real_pipeline(ctx)
    # Every (candidate, fold) pair must have an inner_best_epoch
    assert len(result.inner_best_epochs) == 12
    # Every Stage B run must have a corresponding inner_best_epoch (12/12)
...
# ---- Invariant 9: signoff PASS/PASS_WITH_WARNING ----
...
# ---- Invariant 10: old synthetic path unreachable ----
...
def test_invariant_10_old_synthetic_official_path_unreachable():
    ...
    with patch.object(p44pipe, "run_pipeline", side_effect=explode), \
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ctx = _make_ctx(Path(tmp))
            # If the synthetic mock were reachable, this would raise.
            result = run_real_pipeline(ctx)
...
# ---- Invariant 11: canonical registry unchanged ----
...
# ---- Invariant 12: canonical artifacts unchanged ----
...
# ---- Invariant 13: Test inaccessible ----
...
def test_invariant_13_test_inaccessible():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ctx = _make_ctx(Path(tmp))
        result = run_real_pipeline(ctx)
    # Verify no test rows in the handoff
    if result.phase45_handoff is not None:
        assert result.phase45_handoff.get("test_status") == "NOT_ACCESSED"
    # Verify the orchestrator's scaler audit shows 0 test rows
    # (the result struct exposes these via stage_*_run_ids but the key
    # invariant is that the scaler fit audit passes — implicit in signoff=PASS)
    assert result.signoff_overall_status in {"PASS", "PASS_WITH_WARNING"}