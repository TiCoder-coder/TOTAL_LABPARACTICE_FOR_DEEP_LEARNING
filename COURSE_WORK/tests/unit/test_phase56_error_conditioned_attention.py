...
def test_decile_coverage():
    ...
    for seed in [42, 123, 2026]:
        ...
        # Per seed, decile counts must sum to 2961
...
def test_join_row_count():
    ...
    # Each seed should have observed_join_rows == 2961 * 2 * 4 = 23,688
    for seed in [42, 123, 2026]:
...
def test_continuous_associations_complete():
    ...
    # 3 seeds * 2 layers * 4 heads * 6 metrics * 3 conditionings = 432 rows
...
def test_decile_metric_summary_complete():
    ...
    # 3 seeds * 2 layers * 4 heads * 10 deciles * 6 metrics = 1440 rows
...
def test_decile_profile_by_lag_complete():
    ...
    # 3 seeds * 2 layers * 10 deciles * 72 lags = 4320 rows
...
def test_layer_head_mean_metrics_complete():
    ...
    # 3 seeds * 2 layers * 2961 targets = 17,766 rows
...
def test_layer_head_mean_association_complete():
    ...
    # 3 seeds * 2 layers * 3 conditionings * 6 metrics = 108 rows
...
def test_cross_seed_layer_summary_complete():
    ...
    # 2 layers * 3 conditionings * 6 metrics = 36 rows
    assert len(df) == 2 * 3 * 6
    # SD must be computed from 3 seeds
    valid = df[df["status"] == "OK"]
    if len(valid) > 0:
...
def test_full_matrix_association_complete():
    ...
    # 3 seeds * 2 layers * 4 heads * 2 conditionings * 4 metrics = 192 rows
...
def test_worst_case_present():
    ...
    # 5 ranks * 3 seeds * 2 layers = 30 rows
    assert len(df) == 5 * 3 * 2
...
def test_no_cartesian_subgroup_mining():
    # Findings should not show cartesian subgroup variables like "EXTREME_HIGH x TOF x H3"
    df = pd.read_csv(ECA_DIR / "error_conditioned_attention_findings.csv")
