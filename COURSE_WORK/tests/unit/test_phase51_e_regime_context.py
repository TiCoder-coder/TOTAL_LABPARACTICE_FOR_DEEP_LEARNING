...
class TestSourceIntegrity:
    ...
    def test_03_every_selected_target_joins_phase50_exactly_once(self):
        # Build selected target set across all ranking CSVs
        sel = set()
        ...
        # Every selected target must be in the working table exactly once
        for tid in sel:
            assert tid in wt_by_id

    def test_04_no_selected_target_dropped(self):
        # Count from ranking CSVs and from working-table join (Phase 51-C = 60+20+30+30+10+10 = 160)
        # Per-family uniqueness: ensure each ranking artifact row still resolves
        for rel in PHASE51_C_SHAS:
            rows = _load_csv(rel)
            wt = _load_csv("phase51_target_level_working_table.csv")
    ...
    def test_05_phase50_labels_unchanged(self):
        # Re-running materialize_e must NOT change the working-table regime labels
...
class TestPrevalence:
    ...
    def test_06_global_denominator_2961(self):
        ...
        for r in rows:
            # global_count + (unclassified outside declared labels) is bounded;
            # global_prevalence = global_count / 2961
            assert 0.0 <= float(r["global_prevalence"]) <= 1.0

    def test_07_global_counts_sum_correctly(self):
        rows = _load_csv("regime_overrepresentation.csv")
        # For each (selection_family, seed, regime_family) the sum of counts
        # across declared labels should equal selection_k for that family.
        # Use W2_SHARED_WORST (seed=ALL, K=20)
        for fam in [
    ...
    def test_08_selected_counts_sum_to_K(self):
        rows = _load_csv("regime_overrepresentation.csv")
        # W1_PER_SEED_WORST per seed should sum to 20 for each family
        for seed in SEEDS:
            for fam in [
                "R1_TARGET_LEVEL", "R2_EXTREME_HIGH", "R3_CHANGE_MAGNITUDE",
                "R4_CHANGE_DIRECTION", "R5_TIME_OF_DAY", "R6_DAY_TYPE",
...
class TestEnrichment:
    ...
    def test_11_enrichment_ratio_formula(self):
        ...
        for r in rows:
            if r["status"] == "OK":
                ...
                # Allowance: 9-decimal rounding may amplify tiny floats
    ...
    def test_15_no_cartesian_regime_mining(self):
        # Only canonical 6 families × their declared labels exist
        rows = _load_csv("regime_overrepresentation.csv")
        declared_families = {
            "R1_TARGET_LEVEL", "R2_EXTREME_HIGH", "R3_CHANGE_MAGNITUDE",
            "R4_CHANGE_DIRECTION", "R5_TIME_OF_DAY", "R6_DAY_TYPE",
    ...
    def test_16_no_threshold_recomputation(self):
        # No new fields invented beyond the canonical schema
        rows = _load_csv("regime_overrepresentation.csv")
        expected_keys = {
...
class TestW1SeedSeparation:
    ...
    def test_18_no_60_row_iid_pooling(self):
        # Each W1 entry has selection_k == 20 (NOT 60)
        rows = _load_csv("regime_overrepresentation.csv")
        for r in rows:
            if r["selection_family"] == "W1_PER_SEED_WORST":
...
class TestW2SharedWorstRegimeContext:
    ...
    def test_19_w2_has_20_target_contexts(self):
        ...
        # 6 families × variable labels
        # Verify selection_k = 20
...
class TestPersistenceContext:
    ...
    def test_27_target_alignment_exact(self):
        # Persistence targets must equal working-table targets
        pers = set()
    ...
    def test_29_persistence_does_not_alter_rankings(self):
        # Running materialize_e must not change Phase 51-C ranking SHAs
...
class TestLSTMContext:
    ...
    def test_33_no_fake_lstm_ranking(self):
        # No fabricated LSTM-derived ranking in any Phase 51-E artifact
        files = [
...
class TestSignConsensusContext:
    ...
    def test_36_deterministic_mapping(self):
        # Two runs must produce the same sign-consensus classes
...
class TestSafety:
    ...
    def test_44_no_temporal_context(self):
        # Phase 51-E does NOT have ±6 context. baseline_context has only
    ...
    def test_45_no_input_context(self):
        # No feature columns in any Phase 51-E artifact
    ...
    def test_53_phase51_b_c_d_frozen_unchanged(self):
        # All canonical Phase 51-B/C/D SHAs still match
