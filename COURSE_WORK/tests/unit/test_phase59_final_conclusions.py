...
# ─────────────────────────────────────────────────────────────
# Gate: Artifact completeness (O59.1 – O59.31)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# Conclusion prose — no forbidden phrases
# ─────────────────────────────────────────────────────────────
...
class TestConclusionProse:
    ...
    def test_no_unconditional_causal_phrases(self):
        ...
        # The word "causal" is allowed ONLY in the correct negative forms:
        # "not causal", "NOT causal", "is not causal", "causal attribution" (what it is NOT),
        # "causal contribution" (what it does NOT establish), "causal claims" (in limitations)
        # If "causal" appears, it must be preceded by a negation word
        for m in re.finditer(r'\bcausal\b', text, re.I):
            start = m.start()
            # Get 30 chars before
            before = text[max(0, start-30):start].lower()
            # Allowed if preceded by a negation
            assert any(neg in before for neg in [
                'not ', 'isn', 'don', 'can', 'never ',
                'not\n', 'no ', 'nor ', 'without ',
                'rather ', 'than ', 'instead',
                'as ',  # "as causal" = describing what overclaiming means in limitations
    ...
    def test_no_statistical_significance_without_source(self):
        ...
        # "statistically significant" is allowed ONLY in limitations (as a
        # correctly-scoped warning that formal tests are absent) or in future
        # work / audit prose. It must NOT appear in main conclusion sections.
        # Check that if it appears in non-limitation/non-future prose,
        # it is flagged in the language audit.
        ...
        # The phrase "statistically significant" may appear in limitations
        # correctly describing what is not claimed. The language audit handles
        # this. The prose test just checks it is not used as a positive claim.
        # It is acceptable to not appear at all.
        pass  # language audit handles this
