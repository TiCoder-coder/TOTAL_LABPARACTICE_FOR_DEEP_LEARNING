...
class TestAvgRmseFinalSummary:
    ...
    def test_avg_rmse_fix_present_in_runner_source(self) -> None:
        ...
        # The fix: avg_rmse = float(np.mean([float(r["rmse"]) for r in run_records]))
        # must appear immediately before "Average FINAL_DEV_DIAGNOSTIC RMSE"
        # print statement.
        # Find the exact print statement and verify it is preceded by
        # an avg_rmse assignment.
        # Use a regex-free, simple substring check.
        fix_pattern = (
        ...
        # The fix_pattern must appear before the print_pattern in source.
    ...
    def test_avg_rmse_recomputed_from_run_records(self) -> None:
        ...
        # Simulate the post-training state: 3 run_records with completed rmse.
        run_records = [
        ...
        # Match the value persisted in phase_46_signoff.json
    ...
    def test_runner_has_no_unrelated_undefined_avg_rmse(self) -> None:
        ...
        # Split source by function definitions to find the calling scope.
        # We expect avg_rmse to appear ONLY:
        #   1. Inside write_phase46_artifacts (line ~1367)
        #   2. At the new fix location (line ~2351)
        lines = runner_src.split("\n")
        avg_rmse_lines = [
            (i + 1, line) for i, line in enumerate(lines)
            if "avg_rmse" in line
        ]
        # We expect EXACTLY 5 references: definition (in write_phase46_artifacts),
        # 2 uses inside that function, fix definition + use in main_inner().
