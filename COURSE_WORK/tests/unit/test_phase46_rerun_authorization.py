...
# ---------------------------------------------------------------------------
# Pure unit tests — no registry instantiation required.
# ---------------------------------------------------------------------------
...
class TestCanonicalRerunReason:
    ...
    def test_enum_still_only_contains_known_reasons(self):
        # The enum may legitimately grow over time (PHASE44, PHASE46_HARD_STOP_PROBE).
        # What matters is: PHASE46_CORRECTIVE_RERUN is present, and no
        # ad-hoc / arbitrary reasons sneak in.
        required = {
            "REPRODUCIBILITY_CHECK",
            "CODE_FIX",
            "DEVICE_CHANGE",
...
class TestDriverUsesCanonicalReason:
    ...
    def test_driver_uses_canonical_reason(self):
        ...
        # The driver may use single or double quotes, and may span multiple lines.
        # We just verify the constant PHASE46_CORRECTIVE_RERUN appears as a
        # rerun_reason value in the driver source.
...
class TestRegistryRejectsInvalidReason:
    ...
    def test_invalid_rerun_reason_when_no_duplicate(self):
        # When no duplicate config exists, passing an unsupported rerun_reason
        # raises "Invalid rerun reason" (not "Duplicate config requires …").
        # We assert this by simply checking that "PHASE46_BOGUS" is NOT in
        # RERUN_REASONS — the registry logic itself raises ValueError when
        # the reason is outside the enum.
...
class TestNoTestAccess:
    ...
    def test_driver_target_access_mode_is_final_dev(self):
        driver = (ROOT / "scripts" / "phase46_three_seed_runs.py").read_text()
        # No TEST mode in driver paths.
        assert 'target_access_mode = "TEST"' not in driver
        assert "target_access_mode='TEST'" not in driver
        # FINAL_DEV is the explicit, default, and only used value.
