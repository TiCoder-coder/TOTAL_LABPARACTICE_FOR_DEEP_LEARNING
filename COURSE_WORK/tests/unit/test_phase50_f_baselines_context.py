...
from course_work.error_regime_analysis.materialize_f import main as f_main
...
def test_persistence_global_matches_phase47_global():
    ...
    # Phase 47 global: MAE=21.49..., RMSE=72.92...
    # We sanity-check the values are finite and reasonable, exact match is asserted in:
    # test_persistence_global_matches_phase47_exact below (uses Phase 47 summary)
...
def test_sign_consensus_by_regime_present():
    ...
    rows = list(csv.DictReader(p.open()))
    # 18 labels × 6 source classes (ALL_UNDER, ALL_OVER, ALL_ZERO, TWO_UNDER_ONE_OVER, TWO_OVER_ONE_UNDER, MIXED) = 108
...
def test_sign_consensus_counts_match_phase49_canonical():
    ...
    # Per-target consensus_class counts derived deterministically from wide_table
    # must match Phase 49 aggregated counts after summing across all 6 regime families.
    p49_rows = list(
    ...
    # Each Test target participates in exactly 6 regime families → divide by 6
