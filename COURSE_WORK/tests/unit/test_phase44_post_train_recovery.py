...
from course_work.rolling_origin.finalize import (  # noqa: E402
...
# ---------------------------------------------------------------
# 1. Scoping-bug regression: build_windowpop_with_appliances must
#    be importable at module level only. If a future patch
#    reintroduces a function-local import, this test fails.
# ---------------------------------------------------------------
def test_build_windowpop_with_appliances_is_module_level_import():
    ...
    # The module-level import must exist (top-level `from ... import`).
    assert (
        "from course_work.rolling_origin.appliance_lookup import" in src
    ), "module-level import of appliance_lookup is missing"

    # No function-local `from course_work.rolling_origin.appliance_lookup import`
    # must appear inside any indented block.
    import re
...
# ---------------------------------------------------------------
# 2-3. Completed Stage A and Stage B are discoverable
# ---------------------------------------------------------------
def test_discover_completed_stage_runs_finds_completed_runs():
    ...
    # At minimum, the 12 latest official Stage B runs (0018-0029) must
    # be present.
    expected_b = {
...
# ---------------------------------------------------------------
# 4. Per-pair discovery returns both stages + best_epoch
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# 5-6. Stage B checkpoint strict reload works (no training)
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# 7. failed 0001-0005 cannot be reused (excluded from discovery)
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# 8. finalize mode is idempotent (no new run IDs)
# ---------------------------------------------------------------
def test_finalize_mode_is_idempotent():
    ...
    with tempfile.TemporaryDirectory(prefix="phase44_finalize_idem_") as tmp:
        ...
        # Same candidate / fold / stage counts
    ...
    # Canonical registry must be unchanged
