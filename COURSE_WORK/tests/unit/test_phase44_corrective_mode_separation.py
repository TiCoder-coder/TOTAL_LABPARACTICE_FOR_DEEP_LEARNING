...
class TestScientificBudget(unittest.TestCase):
    ...
    def test_official_accepts_scientific_config(self):
        ...
        # Should not raise — all values are scientific.
...
class TestStageAPayloads(unittest.TestCase):
    ...
    def test_12_stage_a_payloads(self):
        ...
        # If we cannot find scaler audits via probe, fall back to constructing empty proxies
...
class TestPersistenceBundles(unittest.TestCase):
    ...
    def test_persistence_evaluations_present_in_artifact(self):
        # This is the test that documents the missing evidence:
        # the just-quarantined official run had ZERO persistence
        # predictions. The quarantine manifest records this fact.
    ...
    def test_persistence_predictions_missing_was_root_cause(self):
        # After quarantine, the active predictions/ directory should not
        # contain any persistence bundle (because the synthetic pipeline
        # never wrote them).
...
class TestPhase45Handoff(unittest.TestCase):
    ...
    def test_phase45_handoff_reflects_quarantine(self):
        ...
        # In the current state, handoff reflects pre-quarantine values.
        # The quarantine writes a note via signoff; the handoff itself is
        # archived but not overwritten. The next valid run must rebuild it.
...
class TestSyntheticDetection(unittest.TestCase):
    ...
    def test_synthetic_residual_std_too_high(self):
        # If any active prediction has residual std ≈ 30 (synthetic noise),
        # the synthetic detector should flag it. This catches the
        # _synth_predict_for_official() regression.
        ...
        for p in preds_dir.glob("outer_predictions_*.csv"):
            ...
            if len(residuals) > 0:
                # Synthetic detector: std should not be ≈ 30 (the
                # _synth_predict_for_official noise level)
                self.assertFalse(
                    25 <= residuals.std() <= 35,
...
class TestTestFirewall(unittest.TestCase):
    ...
    def test_no_test_rows_in_active_predictions(self):
        ...
        if not preds_dir.exists():
            return
        # We can only do a basic string check; full check requires TEST
        # boundary tracking, which is in populations.py.
        from course_work.rolling_origin.populations import (
...
class TestInvalidRunsCannotBeReused(unittest.TestCase):
    ...
    def test_quarantine_blocks_reuse(self):
        # After quarantine, the archived run_registry CSVs are marked
        # noncanonical. The next official run must not pick them up.
        archive_dirs = [
