...
class TestW1PerSeedWorst:
    ...
    def test_07_w1_tie_break_correct(self):
        # FROZEN contract tie-break: on equal absolute_error, target_id ASC.
        # No target_timestamp tier allowed.
        rows = _load_csv("worst_per_seed_top20.csv")
        for seed in SEEDS:
            seed_rows = sorted([r for r in rows if r["seed"] == seed],
                               key=lambda r: int(r["rank"]))
            ...
            # Also verify the row itself documents the canonical tie-break
...
class TestW2SharedWorst:
    ...
    def test_10_w2_mean_formula(self):
        # Spot-check: mean_abs_error_wh ≈ (seed42+seed123+seed2026)/3
        # Load working table for reference
...
class TestW3SharedAllUnder:
    ...
    def test_22_w3sh_correct_topk(self):
        rows = _load_csv("shared_all_under_top10.csv")
        # Up to K_SHARED_SIGNED rows (could be fewer if not enough eligible)
...
# ── General safety + reproducibility ────────────────────────────────────────
...
class TestSafetyAndReproducibility:
    ...
    def test_26_working_table_sha_stable(self):
        # Re-running materialize_c shouldn't change working table SHA
        # because writers refuse to overwrite (write_bytes_once_or_verify)
    ...
    def test_27_no_random_tiebreak(self):
        # Two runs must produce identical ranking CSVs
        materialize_c.materialize_phase51_c()
        sha1 = hashlib.sha256(
    ...
    def test_31_no_new_inference_artifact(self):
        # No checkpoint-loaded predictions
    ...
    def test_32_no_checkpoint_loading(self):
        # Check there is no .pt / .pkl model in phase51 dir
    ...
    def test_44_no_overlap_jaccard_artifact(self):
        ...
        materialize_c.materialize_phase51_c()
        # Just verify that the manifest does not list any overlap artifact
        manifest = json.loads(
            Path("artifacts/worst_error_analysis/worst_error_ranking_manifest.json").read_text()
        )
        artifacts = manifest.get("artifacts", {})
