...
# Sentinel that mimics the dry-probe — raised by the patched train_stage_a
# at the first Stage A training call, before any training step.
class _TrainingBoundarySentinel(Exception):
    pass


# ---- Test 1: official RunContext accepts canonical fields ----
...
def test_official_runcontext_accepts_canonical_fields():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        # Use the tmp dir for paths to keep them valid
...
# ---- Test 2: obsolete `rehearsal=` keyword is rejected ----
    ...
    with tempfile.TemporaryDirectory() as tmp:
        with pytest.raises(TypeError) as exc_info:
            RunContext(
                ...
                seed=42,
                rehearsal=False,  # OBSOLETE — must raise TypeError
                is_rehearsal=False,
            )
...
# ---- Test 3: every RunContext call site matches dataclass schema ----
    ...
    for relpath in files_to_check:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                # Check that the call is `RunContext(...)`
                func_name = None
...
# ---- Test 4: rehearsal RunContext is still valid ----
...
def test_rehearsal_runcontext_is_still_valid():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert_context_invariants(ctx)  # guards accept this
        assert ctx.is_rehearsal is True
        assert ctx.rehearsal_synthetic is True
        assert ctx.scientific_max_epochs == 2


# ---- Test 5/6/7/8/9/10/11/14: official dry-construction + hard-stop ----
...
def test_official_setup_reaches_first_training_boundary():
    ...
    with tempfile.TemporaryDirectory(prefix="phase44_official_test_") as tmp:
        ...
        ctx = RunContext(
            ...
            scientific_patience=10,
            rehearsal_synthetic=False,
            # Required by official mode: a candidate-aware dataset factory.
        ...
        # Verify the planned counts BEFORE the run
        ...
        assert len(folds) == 3
        # 12 Stage A and Stage B templates, 3 Persistence folds
        assert len(candidates) * len(folds) == 12

        # Patch train_stage_a to raise sentinel at first Stage A call.
    ...
    # Canonical artifact_dir must be untouched
    canonical_after = (
        sorted(p.name for p in canonical_artifact_dir.iterdir())
        if canonical_artifact_dir.exists() else []
    )
