...
# ---------------------------------------------------------------------------
# 1. All three completed Phase46 runs are reusable
# ---------------------------------------------------------------------------
...
class TestFinalizeNoNewRunIDs:
    def test_registry_run_count_unchanged_after_finalize(self) -> None:
        ...
        # Allow: finalize should NOT register new run records
        assert after == before, f"Finalize added {after - before} new registry entries"


# ---------------------------------------------------------------------------
# 3. phase47_release JSON serializable + gates pass
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. FINAL_REFIT metric semantics
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. FINAL_SCALING identity across seeds
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. Population fingerprint identity
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7. Exactly 3 completed seeds (no more, no less)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 8. No artifact-path collisions between the 3 runs
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 9. Test access = NOT_ACCESSED everywhere
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 10. Training guard installed by finalize
# ---------------------------------------------------------------------------
...
class TestFinalizeTrainingGuard:
    def test_optimizer_step_raises_during_finalize(self) -> None:
        ...
        # Run a separate script invocation that doesn't try to train
        # (this test is a sanity check; actual finalize run already guards)
...
# ---------------------------------------------------------------------------
# 12. Phase47 release blocked when Phase46 incomplete
# ---------------------------------------------------------------------------
...
class TestIdempotency:
    def test_archive_dir_created(self) -> None:
        ...
        # At least one timestamped archive subdir
        assert any(archive_root.iterdir()), "Archive directory empty"


# ---------------------------------------------------------------------------
# 14. Strict checkpoint reload works for all 3 seeds
# ---------------------------------------------------------------------------
...
class TestStrictCheckpointReload:
    ...
    def test_checkpoint_loads_with_torch(self, seed: int) -> None:
        ...
        # Load on CPU to avoid MPS init issues in CI
        ...
        keys = list(sd.keys())
        # Transformer model has encoder.layers in state dict
