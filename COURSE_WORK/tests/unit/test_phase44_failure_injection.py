...
def test_missing_transformer_shortlist_fails_gracefully():
    ...
    # With the handoff present, only LSTM loads (1 candidate); signoff fails
    # because C11 (recommended_transformer_consistent) cannot be satisfied
    # (no Transformer candidates to rank).
...
def test_missing_lstm_handoff_fails_gracefully():
    ...
    after = {p.name for p in canonical.iterdir()} if canonical.exists() else set()
    assert before == after
    # Without LSTM handoff, only 3 Transformer candidates are loaded.
    # The orchestrator returns exit_code=0 with signoff=PASS_WITH_WARNING
    # because the LSTM handoff is optional (degraded mode).
    assert result.n_candidates == 3
    assert result.signoff_overall_status in {"PASS", "PASS_WITH_WARNING"}
    # The C01 (LSTM fingerprint) check is the only soft warning when LSTM
    ...
    # No exception was raised; orchestrator handled it gracefully
...
def test_zero_epoch_budget_fails_gracefully():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # The guard allows 0 if rehearsal=False? No, only >5 is rejected.
        # Try >5 (the rehearsal ceiling)
...
def test_invalid_artifact_dir_is_reported_as_failure():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ctx = _ctx(Path(tmp))
        # Provide an artifact_dir on a read-only path
        ctx.artifact_dir = Path("/nonexistent_readonly_dir_9999/artifacts")
        result = run_real_pipeline(ctx)
    after = {p.name for p in canonical.iterdir()} if canonical.exists() else set()
    assert before == after
    # Result may succeed or fail depending on whether it writes anything; the
    # important thing is that canonical was not mutated.
...
def test_orchestrator_never_writes_under_canonical_artifact_dir_on_failure():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        # Cause failure by breaking transformer_shortlist
        ctx = _ctx(Path(tmp))
...
def test_synthetic_official_mock_is_unreachable():
    ...
    with patch.object(p44pipe, "run_pipeline", side_effect=explode), \
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ctx = _ctx(Path(tmp))
            # If the synthetic mock were reachable, run_real_pipeline would
            # raise. The real orchestrator must complete normally.
            result = run_real_pipeline(ctx)

    # The real orchestrator succeeded WITHOUT calling the synthetic mock.
...
def test_fold_local_bundle_uses_canonical_ys1_statistics():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Provenance: each stage MUST be fit on its allowed population
        # and produce statistics that may differ from the canonical global YS1.
        seen_means = set()
        seen_stds = set()
        for stage in ("A", "B"):
            bundle, _ = fit_fold_local_scaler(
            ...
            # The bundle MUST have a fitted scaler object (schema contract)
            d = bundle.as_dict()
            assert d["option"] == "YS1"
            assert d["scaling_version"] in ("SCALING-v1", "FINAL_SCALING-v1")
        ...
        # Across all 3 folds we must observe at least 2 distinct y_mean
        # values across the 6 (3 folds × 2 stages) fold-stage Y scalers.
        # This is a strong indicator that values were NOT copied from a
