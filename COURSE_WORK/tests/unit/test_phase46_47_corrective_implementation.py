...
# ============================================================================
# Constants — canonical Phase 45 lock
# ============================================================================
...
class TestLockIdentitySplit:
    ...
    def test_locked_config_fp_backward_compat_alias(self):
        ...
        # The alias must point to the config fingerprint (its original value).
    ...
    def test_distinct_constants_exported(self):
        ...
        assert hasattr(fte, "LOCKED_CONFIG_FP")  # back-compat alias


# ============================================================================
# Fix #5 + #6: Phase 47 runner writes the correct final_lock_sha256
# ============================================================================
class TestPhase47RunnerLockIdentity:
    def test_p47_runner_imports_lock_constants(self):
        # Use importlib to import by file path so we can confirm import.
    ...
    def test_p47_runner_writes_lock_sha_not_config_fp_into_contract(self):
        ...
        # Must contain the new constant
        assert "LOCKED_FINAL_LOCK_SHA256" in p47
        # The contract-write line uses LOCKED_FINAL_LOCK_SHA256 explicitly
...
class TestPhase46NoTRC0Fallback:
    def test_p46_runner_refuses_TR_C0_PRIMARY_fallback(self):
        ...
        # We must NOT find the silent fallback pattern
    ...
    def test_p46_runner_explicit_guard_against_TR_C0_PRIMARY(self):
        ...
        # The guard must be a STOP/exit, not a silent acceptance
        ...
        # The guard must trigger sys.exit(2)
    ...
    def test_p46_runner_handoff_missing_candidate_id_stops(self):
        ...
        p46 = (ROOT / "src/course_work/scripts/p46_three_seed_runs.py").read_text()
        # The new code path includes explicit checks
        assert "Refusing to fall back" in p46 or "cannot be resolved" in p46


# ============================================================================
# Fix #2 + #3: Phase 46 runner sources FINAL_REFIT_EPOCHS strictly from Phase 45
# ============================================================================
class TestPhase46EpochSource:
    ...
    def test_p46_runner_no_silent_50_epoch_default(self):
        ...
        p46 = (ROOT / "src/course_work/scripts/p46_three_seed_runs.py").read_text()
        # The old `int(training.get("max_epochs", 50))` pattern is gone
        # We allow 50 to appear only in error/comment context
...
# ============================================================================
# Fix #11: Lifecycle-split pre-Test gate (PRETRAIN + POSTRUN)
# ============================================================================
class TestLifecycleSplitGate:
    ...
    def test_pretrain_does_not_require_phase47_test_release(self):
        ...
        mod = self._load_gate_module()
        # Iterate PRETRAIN gates and inspect each function's source code
        # for references to phase47_test_release / PHASE47_RELEASE_PATH.
        import inspect
        for fn in mod.PRETRAIN_GATES:
    ...
    def test_postrun_gates_pass_when_checkpoints_present(self):
        ...
        """
        mod = self._load_gate_module()
        # First check that RG2 (checkpoint binaries exist) PASSES
        ...
        # Verify RG1 (3 metadata sidecars exist) PASSES
    ...
    def test_postrun_gates_fail_on_stale_historical_run_ids(self):
        ...
        # After successful 3-seed Phase 46 completion, RG3 must PASS.
        result = rg3()
        assert result.status == "PASS", (
            f"RG3 must PASS after Phase 46 completion; got {result.status}: "
            f"{result.detail}"
        )
        # The accepted run_ids must be the 3 corrected RUN_TR_FSD_0256_* IDs.
        ...
        # None of the run_ids may be in EXCLUDED_HISTORICAL_RUN_IDS.
    ...
    def test_postrun_phase47_release_not_a_prerequisite_gate(self):
        ...
        mod = self._load_gate_module()
        import inspect
        # Search for any gate function that gates ON the release being released=true.
        for fn in mod.POSTRUN_GATES:
            src = inspect.getsource(fn)
            # No POSTRUN gate should check `release.get("released") is True` as a gate condition;
            # that's the responsibility of the release generator itself.
    ...
    def test_no_skip_semantics_in_lifecycles(self):
        ...
        for fn in mod.PRETRAIN_GATES + mod.POSTRUN_GATES:
            src = inspect.getsource(fn)
            # No call to .skip(...) within gate functions
    ...
    def test_gate_run_id_threshold_no_longer_required(self):
        ...
        for fn in mod.POSTRUN_GATES:
            src = inspect.getsource(fn)
            # No hard-coded numeric run_id prefix enforcement.
    ...
    def test_no_test_access_in_gate_runner(self):
        ...
        path = ROOT / "src/course_work/scripts/p46_corrective_pretrain_gate.py"
        text = path.read_text()
        # Strip docstrings via AST to avoid false positives from documentation.
        tree = ast.parse(text)
        ...
        # The gate may reference "build_test_dataset(" as a forbidden-pattern token
        # inside a list/tuple — that is fine.
    ...
    def test_historical_test_log_preserved_by_gate(self):
        ...
        path = ROOT / "src/course_work/scripts/p46_corrective_pretrain_gate.py"
        text = path.read_text()
        # No destructive operations on the .archive directory
        assert ".unlink" not in text or ".unlink" in text and "final_test_discrepancies" not in text
        # Must reference the .archive directory for preservation verification
...
# ============================================================================
# Fix #12: Phase 47 archive script
# ============================================================================
class TestPhase47ArchiveScript:
    ...
    def test_p47_archive_script_no_test_access(self):
        ...
        path = ROOT / "src/course_work/scripts/p47_archive_current_evidence.py"
        text = path.read_text()
        # Must NOT have build_test_dataset or TestLoader
        assert "build_test_dataset" not in text
        assert "TestLoader" not in text
        # Must NOT have training references
...
# ============================================================================
# Fix #14: Protocol drift guards
# ============================================================================
class TestProtocolDriftGuards:
    def test_p46_runner_no_warm_start(self):
        p46 = (ROOT / "src/course_work/scripts/p46_three_seed_runs.py").read_text()
        # The runtime should not enable warm start
        assert 'warm_start = True' not in p46
        # 'warm-start' / 'loaded_state_dict' should not appear in active context
        for line in p46.split("\n"):
            stripped = line.strip()
...
# ============================================================================
# Phase 47 release gate enforcement (G16)
# ============================================================================
class TestPhase47ReleaseGateEnforced:
    def test_p47_runner_requires_released_phase47_release(self):
        p47 = (ROOT / "src/course_work/scripts/p47_final_test_evaluation.py").read_text()
        # The runner must verify phase47_test_release.released == true
    ...
    def test_p47_pretest_gate_uses_distinct_constants(self):
        path = ROOT / "src/course_work/scripts/p47_pretest_gate.py"
        text = path.read_text()
        # It must compare to LOCKED_FINAL_LOCK_SHA256 (not LOCKED_CONFIG_FP)
        assert "LOCKED_FINAL_LOCK_SHA256" in text
        assert "ckpt.final_lock_sha256 == LOCKED_CONFIG_FP" not in text


# ============================================================================
# Prediction bundle persistence (Fix #13)
# ============================================================================
class TestPredictionBundlePersistence:
    ...
    def test_writer_emits_per_seed_csv(self):
        ...
        from course_work.final_test_evaluation import writers
        # Just check the function is callable with the expected args
        import inspect
...
# ============================================================================
# Final-lock vs config-fingerprint separation everywhere
# ============================================================================
class TestLockSeparationEverywhere:
    def test_no_writer_mixes_lock_and_config(self):
        ...
        # Find lines that write the final_lock_sha256 / final_lock_hash field
        # and verify they use LOCKED_FINAL_LOCK_SHA256, NOT LOCKED_CONFIG_FP
        bad = re.findall(
    ...
    def test_no_p47_pretest_gate_mixes_lock_and_config(self):
        ...
        # The OLD wrong usage must not be present
...
# ============================================================================
# Lifecycle-split fixture tests (Part 2F-B requirement)
# ============================================================================
class TestLifecycleSplitFixture:
    ...
    def test_postrun_passes_when_corrected_checkpoints_exist(self):
        ...
        for fn in mod.POSTRUN_GATES:
            results.append(fn())
        # RG2 (checkpoint binaries exist) must PASS
    ...
    def test_postrun_can_pass_when_corrected_artifacts_provided(self, tmp_path):
        ...
        # Build synthetic corrected per-seed checkpoint metadata + .pt binaries
        # + a synthetic phase_46_signoff.json in a temporary directory.
        ...
        # Per-seed corrected metadata + tiny checkpoint binaries (with distinct model_state_sha256).
        # Three DIFFERENT model_state_sha256 values are intentional: the plan
        # explicitly says different checkpoint model-state SHA values may be
        # recorded as a diagnostic, and "all three SHAs must be distinct" is NOT
        # a scientific correctness gate.
        per_seed = {}
        for seed in [42, 123, 2026]:
            ...
            meta = {
                ...
                # Part 2G-N: per-seed schema now uses `config_sha256` (canonical
                # Phase 46 fingerprint field name), `population_fingerprint`
                # (FINAL_DEV manifest identity), `recipe_sha256`. The
                # `final_lock_sha256` field is intentionally omitted from
                # per-seed metadata (RG5 derives it from the signoff).
        ...
        # Synthetic phase_46_signoff.json (lock-consistent, no discrepancies).
        # Part 2G-N: must include all canonical Phase 46 identities that
        # RG5 and RG6 derive from the signoff.
        signoff = {
            "overall_status": "PASS",
            "status": "PASS",
            "discrepancies": [],
        ...
        # Patch the module's module-level paths to point to the synthetic dir.
        # This is the ONLY way to test the gate against a controlled fixture
        # without writing to the real artifacts directory.
        ...
        # The phase47 release path is intentionally not created — it should NOT
        # be a prerequisite for POSTRUN.
        mod.PHASE47_RELEASE_PATH = synthetic_artifacts / "phase47_test_release.json"
        try:
            # Run each POSTRUN gate against the synthetic fixture.
            failed_gates = []
            for fn in mod.POSTRUN_GATES:
                result = fn()
    ...
    def test_historical_test_log_preserved_by_postrun_gate(self):
        ...
        # Verify the .archive directory exists and has historical events.
        ...
        # Now run the POSTRUN gate — it must NOT modify the directory.
        mod = self._load_gate_module()
        for fn in mod.POSTRUN_GATES:
    ...
    def test_postrun_release_true_only_after_postrun_pass(self, tmp_path):
        ...
        mod = self._load_gate_module()
        # Confirm no POSTRUN gate checks phase47_test_release.released == true
        import inspect
        for fn in mod.POSTRUN_GATES:
            src = inspect.getsource(fn)
...
# ============================================================================
# Fix: Phase 46 ROOT artifact path resolution (Part 2G-B)
# ============================================================================
class TestPhase46ArtifactRootResolution:
    ...
    def test_runner_root_is_course_work_project_root(self):
        ...
        # ROOT must contain both `artifacts` and `src` directories.
        ...
        # ROOT must NOT contain src/course_work (i.e. must not be the python package dir)
    ...
    def test_no_absolute_paths_in_runner(self):
        ...
        # No /Users/, no /home/, no C:\\ paths anywhere.
        assert "/Users/" not in text, (
            "Runner hard-codes a /Users/ absolute path. Must use the canonical "
            "ROOT resolver."
        )
        # Strip comments and docstrings via AST so explanatory comments don't
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef, ast.Module)):
                if (node.body and isinstance(node.body[0], ast.Expr)
                    ...
                    node.body[0].value.value = ""
        active = ast.unparse(tree)
        # The buggy resolution pattern must no longer be in ACTIVE code.
    ...
    def test_loading_runner_does_not_execute_scientific_code(self):
        ...
        mod = self._load_runner_module()
        # Module-level constants are loaded; scientific main() is gated by
        # `if __name__ == "__main__":` and is NOT invoked at import time.
        # We assert that no Phase 45 lock file was modified.
        ...
        # Re-load to make sure repeated imports don't have side effects.
