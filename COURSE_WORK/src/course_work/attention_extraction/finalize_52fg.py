...
from course_work.attention_extraction.sources import (
    PHASE_DIR_REL,
    RAW_DIR_REL,
    SEEDS,
    LOOKBACK,
...
from course_work.attention_extraction.writers import write_csv_atomic, write_json_atomic
from course_work.attention_extraction.contract import write_dense_case_order
...
def main() -> None:
    ...
    # ── Summary (write before inventory so it counts) ────────────────
    # use a pre-inventory status: pass all 37 items that we are ABOUT to write
    ...
    # ── O52 inventory (last; reads on-disk state) ─────────────────────
