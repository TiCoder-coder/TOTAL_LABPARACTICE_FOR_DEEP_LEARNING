...
def last_query_summary(
    ...
    for n in range(N):
        ...
        for layer in range(NUM_LAYERS):
            for head in range(H):
                a = last_query[n, layer, head, :]  # [LOOK]
                # Lag steps: position 0 → lag LOOK, position LOOK-1 → lag 1
                lag_steps = LOOK - np.arange(LOOK)  # [LOOK]
                ...
                # top1 deterministic tie rule (newest source wins ties)
                max_w = float(a.max())
                tied_positions = np.flatnonzero(np.isclose(a, max_w))
                # newest = largest index among ties
                ...
                for name, steps in RECENT_WINDOWS_STEPS.items():
                    ...
                    # lag <= eff_steps ⇒ position >= LOOK - eff_steps
    ...
    for n in range(B):
        ...
        for layer_idx0, layer_attn in enumerate(attention_layers_batch):
            for head_idx0 in range(layer_attn.shape[1]):
                A = layer_attn[n, head_idx0, :, :]  # [L, L]
                # mean_query_entropy
                # compute per-query entropy
                row_sums = A.sum(axis=-1)  # [L]
                # If any row-sum is not ~1, fall back to softmax over keys for entropy
                # but DO NOT modify the raw matrix; only compute derived entropy.
                ...
                # mean_self_attention_weight = mean_q A[q,q]
                diag = np.diag(A)
                self_mass = float(diag.mean())

                # mean_absolute_query_source_distance = mean_q sum_s A[q,s] * |q-s|
                ...
                # backward_or_same_mass = mean_q sum_{s<=q} A[q,s]
                # forward_within_input_mass = mean_q sum_{s>q} A[q,s]
                # triu (k=0): s <= q. tril (k=-1): s < q.
                # mean over all (q,s) pairs in mask.
    ...
    for k_pos in range(dense_attention.shape[0]):
        ...
        for layer_idx0 in range(dense_attention.shape[1]):
            for head_idx0 in range(dense_attention.shape[2]):
                dense_row = dense_attention[k_pos, layer_idx0, head_idx0, -1, :]  # [L]
                lq_row = last_query[all_idx, layer_idx0, head_idx0, :]  # [L]
