...
def write_csv_atomic(
    ...
    if fieldnames is None:
        ...
        # include any extra keys from subsequent rows
        seen = set(keys)
        for r in rows[1:]:
            for k in r.keys():
                if k not in seen:
...
def write_npz_atomic(arr_dict: dict[str, np.ndarray], fp: Path) -> None:
    ...
    """
    fp.parent.mkdir(parents=True, exist_ok=True)
    # Use a temp dir so np.savez creates files there, then move the .npz
    ...
    try:
        ...
        pass  # dir may not be empty if multiple writes in same batch
    try:
        os.chmod(fp, 0o444)
    except (OSError, PermissionError):
        pass
...
def reload_verify_npz(
    ...
    for name, (shape, dtype) in expected_arrays.items():
        ...
        # dtype matching: for U-strings, accept any U-string
