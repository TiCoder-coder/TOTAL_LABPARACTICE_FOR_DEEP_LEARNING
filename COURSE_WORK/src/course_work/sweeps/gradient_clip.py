...
class GradientClipCondition:
    ...
    condition_id: str
    clip_enabled: bool
    clip_method: str | None  # None, "global_norm"
    max_norm: float | None
    norm_type: int | None
    execution_mode: str
...
# Phase 39 Conditions
# GC0: clipping OFF, finite-gradient guard ON
# GC1: global norm clipping, max_norm=1.0, after backward, before optimizer.step()
CONDITIONS = (
...
def verify_gc1_reference(root: Path) -> dict[str, Any]:
    ...
    # Verify gradient clipping config
    ...
    # Verify GC1 = clipping enabled with max_norm=1.0
...
def get_phase_39_frozen_config(root: Path) -> dict[str, Any]:
    ...
    # Build frozen configuration in the format expected by run_single_condition.py
    ...
    return {
        ...
        # Additional fields for Phase 39
...
def verify_phase_39_preflight(root: Path) -> dict[str, Any]:
    ...
    # 3. Verify GC1 has clipping enabled
...
def create_phase_39_manifest(root: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "gc1_clipping_fraction": 0.929375531011045,  # From S16 E100 run
    }
...
def prepare_phase_39_condition(condition_id: str, project_root: Path | None = None) -> dict[str, Any] | None:
    ...
    # Get frozen config in the format expected by run_single_condition.py
