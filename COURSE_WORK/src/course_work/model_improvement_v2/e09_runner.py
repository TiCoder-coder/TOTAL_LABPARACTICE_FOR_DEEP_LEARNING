"""E09 SGD-with-momentum LR/weight-decay audit with immutable E01 control."""
...
EXPERIMENT_ID = "E09"
EXECUTION_TRACK = "MODEL_IMPROVEMENT_V2_E09"
REGISTRY_NAMESPACE = "V2_E09"
SUPPORTED_SEED = 42
...
CHALLENGER_LRS = (1e-3, 3e-3, 1e-2)
CHALLENGER_WEIGHT_DECAYS = (0.0, 1e-4)
CHALLENGER_GRID = tuple(
    (learning_rate, weight_decay)
    for learning_rate in CHALLENGER_LRS
    for weight_decay in CHALLENGER_WEIGHT_DECAYS
)
CANDIDATE_IDS = {
    (1e-3, 0.0): "TR_C2_ALT_LOOKBACK_SGDM_LR_1E_MINUS_3_WD_0",
    (1e-3, 1e-4): "TR_C2_ALT_LOOKBACK_SGDM_LR_1E_MINUS_3_WD_1E_MINUS_4",
    (3e-3, 0.0): "TR_C2_ALT_LOOKBACK_SGDM_LR_3E_MINUS_3_WD_0",
    (3e-3, 1e-4): "TR_C2_ALT_LOOKBACK_SGDM_LR_3E_MINUS_3_WD_1E_MINUS_4",
    (1e-2, 0.0): "TR_C2_ALT_LOOKBACK_SGDM_LR_1E_MINUS_2_WD_0",
    (1e-2, 1e-4): "TR_C2_ALT_LOOKBACK_SGDM_LR_1E_MINUS_2_WD_1E_MINUS_4",
}
OPTIMIZER_CONFIG = {"momentum": 0.9, "nesterov": False}
CONFIG_RELATIVE_PATH = Path(
    "artifacts/model_improvement_v2/experiments/E09/e09_config_snapshot.json"
)
E01_RELATIVE_ROOT = Path("artifacts/model_improvement_v2/experiments/E01")


class E09PreflightError(RuntimeError):
    pass
...
def _read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise E09PreflightError(f"Cannot load {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise E09PreflightError(f"Expected JSON object: {path}")
    return value
...
def load_e09_config(root: Path) -> dict[str, Any]:
...
def _validate_control(
    ...
    if control.get("source_experiment") != "E01":
        raise E09PreflightError("E09 control source must be E01")
    if control.get("reuse_policy") != "CONTROL_REUSED_FROM_E01_READ_ONLY":
        raise E09PreflightError("E09 control must be reuse-only")
    if control.get("training_required") is not False:
        raise E09PreflightError("E09 must not retrain the E01 control")
    if control.get("pooled_metrics") != CONTROL_METRICS:
        raise E09PreflightError("E01 control metrics mismatch")
    if control.get("test_status") != "NOT_ACCESSED":
        raise E09PreflightError("E01 control violates Test firewall")
    if control.get("config") != e01_config:
        raise E09PreflightError("E09 control config differs from E01")
    if control.get("config_fingerprint") != compute_config_fingerprint(e01_config):
        raise E09PreflightError("E09 control fingerprint mismatch")
    for name, expected_sha in control.get("artifact_sha256", {}).items():
        ...
        if not artifact.is_file() or _sha256(artifact) != expected_sha:
            raise E09PreflightError(f"E01 immutable control artifact changed: {name}")
    execution = _read_json(root / E01_RELATIVE_ROOT / "e01_execution_manifest.json")
    if execution.get("test_status") != "NOT_ACCESSED":
        raise E09PreflightError("E01 execution manifest violates Test firewall")
    if execution.get("run_ids") != control.get("run_ids"):
        raise E09PreflightError("E01 control run ledger mismatch")
    run_ids = [
        run_id
    ...
    if len(run_ids) != 6 or len(set(run_ids)) != 6 or not all("_E01_" in x for x in run_ids):
        raise E09PreflightError("E01 control requires six canonical run IDs")
...
def _validate_predecessor(root: Path, document: Mapping[str, Any]) -> None:
    ...
    if (
        predecessor.get("experiment_id") != "E08"
        or predecessor.get("decision") != "RETAIN_E01_ADAMW"
        or predecessor.get("best_sgd_learning_rate") != 3e-3
        or predecessor.get("best_sgd_weight_decay") != 1e-3
        or predecessor.get("best_sgd_rmse_wh") != 60.9304324745535
        or predecessor.get("test_status") != "NOT_ACCESSED"
    ):
        raise E09PreflightError("E08 predecessor decision mismatch")
    path = root / "artifacts/model_improvement_v2/experiments/E08/e08_sgd_lr_ablation_comparison.json"
    if not path.is_file() or predecessor.get("artifact_sha256") != _sha256(path):
        raise E09PreflightError("E08 immutable comparison artifact changed")
    evidence = _read_json(path)
    rows = evidence.get("results", [])
    best_sgd = min(
        (row for row in rows if row.get("source") == "E08_TRAINED_CHALLENGER"),
        key=lambda row: float(row["pooled_metrics"]["rmse_wh"]),
    )
    if (
        evidence.get("decision", {}).get("action") != "RETAIN_E01_ADAMW"
        or best_sgd.get("learning_rate") != 3e-3
        or best_sgd.get("weight_decay") != 1e-3
        or best_sgd.get("pooled_metrics", {}).get("rmse_wh") != 60.9304324745535
        or evidence.get("test_status") != "NOT_ACCESSED"
    ):
        raise E09PreflightError("E08 accepted decision/result/Test status mismatch")


def _expected_candidate_config(
    e01_config: Mapping[str, Any], learning_rate: float, weight_decay: float
) -> dict[str, Any]:
...
def validate_e09_document(
    document: Mapping[str, Any], root: Path
) -> dict[str, Any]:
    if document.get("experiment_id") != EXPERIMENT_ID:
        raise E09PreflightError("Wrong E09 experiment identity")
    if document.get("track") != "MODEL_IMPROVEMENT-v2":
        raise E09PreflightError("Wrong E09 track")
    if document.get("objective") != "SGDM_LEARNING_RATE_AND_WEIGHT_DECAY_AUDIT":
        raise E09PreflightError("Wrong E09 objective")
    primary = document.get("primary_change", {})
    if primary.get("factor") != "SGDM_FAMILY_WITH_PREDECLARED_LR_AND_WEIGHT_DECAY":
        raise E09PreflightError("Wrong E09 primary factor")
    if (
        primary.get("candidate_learning_rates") != list(CHALLENGER_LRS)
        or primary.get("candidate_weight_decays") != list(CHALLENGER_WEIGHT_DECAYS)
    ):
        raise E09PreflightError("Wrong E09 LR/weight-decay grid")
    ...
    if not isinstance(candidates, list) or len(candidates) != 6:
        raise E09PreflightError("E09 requires exactly six challengers")
    ...
    for candidate in candidates:
        ...
        wd = candidate.get("weight_decay")
        config = candidate.get("config")
        if (lr, wd) not in CHALLENGER_GRID or cid != CANDIDATE_IDS[(lr, wd)]:
            raise E09PreflightError("E09 challenger ID/LR/weight-decay mismatch")
        if not isinstance(config, dict) or config != _expected_candidate_config(e01_config, lr, wd):
            raise E09PreflightError(f"E09 config mismatch for {cid}")
        ...
        if not changed or any(
            ...
            raise E09PreflightError(f"E09 out-of-factor config change for {cid}")
        training = config["training"]
        if (
            training.get("optimizer_name") != "SGD"
            or training.get("optimizer_config") != OPTIMIZER_CONFIG
            or training.get("weight_decay") != wd
            or training.get("scheduler_name") is not None
            or training.get("scheduler_config") is not None
        ):
            raise E09PreflightError(f"E09 SGDM contract mismatch for {cid}")
        if config["model"].get("prediction_formulation", "DIRECT") != "DIRECT":
            raise E09PreflightError("E09 must use DIRECT prediction")
        fingerprint = compute_config_fingerprint(config)
        if candidate.get("config_fingerprint") != fingerprint:
            raise E09PreflightError(f"E09 fingerprint mismatch for {cid}")
        if candidate.get("initialization_policy") != "FRESH_FROM_SEED_42_NO_CHECKPOINT_LOAD":
            raise E09PreflightError(f"E09 initialization mismatch for {cid}")
        seen_ids.add(cid)
        seen_pairs.add((lr, wd))
        seen_fingerprints.add(fingerprint)
        changed_paths[cid] = list(changed)
    if seen_ids != set(CANDIDATE_IDS.values()) or seen_pairs != set(CHALLENGER_GRID):
        raise E09PreflightError("E09 challenger matrix is incomplete or duplicated")
    if len(seen_fingerprints) != 6:
        raise E09PreflightError("E09 candidate fingerprints must be unique")
    ...
    if tuple(document.get("feature_order", ())) != tuple(get_feature_list(FEATURE_VARIANT)):
        raise E09PreflightError("E09 FS2_TF1 feature order mismatch")
    ...
    if fixed != expected_fixed:
        raise E09PreflightError("E09 fixed scientific contract mismatch")

    orchestration = document.get("v2_orchestration", {})
    expected_orchestration = {
        "registry_namespace": REGISTRY_NAMESPACE,
        "execution_track": EXECUTION_TRACK,
        "shared_pretest_cache_key": "MODEL_IMPROVEMENT_V2_E09_SHARED_PRETEST",
        "training_candidate_count": 6,
        "control_retrained": False,
        "challengers_only_training": True,
        "expected_stage_a_runs": 18,
        "expected_stage_b_runs": 18,
        "expected_total_training_runs": 36,
    ...
    for key, expected in expected_orchestration.items():
        if orchestration.get(key) != expected:
            raise E09PreflightError(f"E09 orchestration mismatch: {key}")
    ...
    for key in ("artifact_root", "registry_root", "run_root"):
        ...
        if not isinstance(value, str) or "/experiments/E09" not in value:
            raise E09PreflightError(f"E09-owned path required: {key}")
        assert_v2_artifact_path(value)

    recovery = document.get("recovery_policy", {})
    expected_recovery = {
        "mode": "NO_RETRAIN_STAGE_C_ONLY",
        "expected_completed_run_count": 36,
        "expected_stage_b_checkpoint_count": 18,
    ...
    if recovery != expected_recovery:
        raise E09PreflightError("E09 recovery policy mismatch")
    ...
    if selection != {
        ...
        raise E09PreflightError("E09 selection policy mismatch")
    ...
    if not frozenset({
        ...
        raise E09PreflightError("E09 Stage-C flatten scope mismatch")
    if V2_SHARED_PRETEST_CACHE_KEYS.get(EXECUTION_TRACK) != orchestration["shared_pretest_cache_key"]:
        raise E09PreflightError("E09 shared pre-Test cache is not wired")
    ...
    return {
        ...
        "training_candidate_count": 6,
        "candidate_ids": [CANDIDATE_IDS[pair] for pair in CHALLENGER_GRID],
        "candidate_grid": [
            {"learning_rate": lr, "weight_decay": wd}
            for lr, wd in CHALLENGER_GRID
        ],
        "optimizer": "SGD",
...
def run_preflight(root: Path | None = None) -> dict[str, Any]:
    ...
    return validate_e09_document(load_e09_config(resolved), resolved)


def build_e09_candidates(document: Mapping[str, Any]):
    ...
    for item in document["training_candidates"]:
        ...
        specs.append(CandidateSpec(
            ...
            lookback_steps=72,
            boundary_protocol="WB0_CONTEXT_CARRY_OVER",
            source_phase="V2_E09_IMMUTABLE_SNAPSHOT",
            candidate_role="SGDM_LR_WEIGHT_DECAY_CHALLENGER",
        ))
    return tuple(specs)
...
def build_e09_run_context(root: Path, document: Mapping[str, Any]):
    ...
    candidates = build_e09_candidates(document)
    artifact_dir = root / "artifacts/model_improvement_v2/experiments/E09"
    cache: dict[str, Any] = {}
    ...
    def dataset_factory(candidate_spec, _fold):
        if (
            ...
            raise E09PreflightError("E09 dataset factory rejects unknown candidate")
        ...
        if audit.test_rows_read or audit.test_target_ids_seen:
            raise PermissionError("E09 Test firewall failed")
        return dataset
...
def _run_dir(root: Path, run_id: str) -> Path:
    return root / "artifacts/model_improvement_v2/experiments/E09/runs" / run_id


def _audit_resume_contract(root: Path) -> dict[str, Any]:
    registry_path = root / "artifacts/model_improvement_v2/experiments/E09/registry/experiment_registry.jsonl"
    if not registry_path.is_file():
        raise E09PreflightError("E09 registry missing for resume")
    records = [
        json.loads(line)
        for line in registry_path.read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    if len(records) != 36 or len({row.get("run_id") for row in records}) != 36:
        raise E09PreflightError("E09 resume requires exactly 36 unique completed runs")
    ledger: dict[str, str] = {}
    epochs: dict[str, int] = {}
    ...
    for record in records:
        ...
        if record.get("status") != "COMPLETED":
            raise E09PreflightError(f"E09 resume run is not COMPLETED: {run_id}")
        if cid not in CANDIDATE_IDS.values() or stage not in {
            "RO1_A", "RO2_A", "RO3_A", "RO1_B", "RO2_B", "RO3_B"
        }:
            raise E09PreflightError(f"E09 resume run identity mismatch: {run_id}")
        if record.get("config", {}).get("data", {}).get("target_access_mode") == "TEST":
            raise E09PreflightError(f"E09 resume Test-scoped run rejected: {run_id}")
        best_epoch = record.get("best_epoch")
        if not isinstance(best_epoch, int) or best_epoch < 1:
            raise E09PreflightError(f"E09 resume invalid best_epoch: {run_id}")
        key = f"{cid}:{stage}"
        if key in ledger:
            raise E09PreflightError(f"E09 duplicate resume key: {key}")
        ledger[key] = run_id
        ...
        if not checkpoint.is_file() or checkpoint.stat().st_size <= 0:
            raise E09PreflightError(f"E09 checkpoint missing/empty: {run_id}")
        ...
        if len(registered) != 1 or registered[0].get("sha256") != actual_sha:
            raise E09PreflightError(f"E09 checkpoint registration/SHA mismatch: {run_id}")
    ...
    if set(ledger) != expected_keys or len(checkpoint_sha256) != 18:
        raise E09PreflightError("E09 resume ledger/checkpoint matrix mismatch")
    for cid in CANDIDATE_IDS.values():
        for fold in (1, 2, 3):
            if epochs[f"{cid}:RO{fold}_A"] != epochs[f"{cid}:RO{fold}_B"]:
                raise E09PreflightError(f"E09 Stage A/B epoch mismatch: {cid}/RO{fold}")
    return {
        "status": "PASS",
        "completed_run_count": 36,
        "stage_a_run_count": 18,
        "stage_b_run_count": 18,
        "stage_b_checkpoint_count": 18,
...
def build_e09_resume_context(root: Path, document: Mapping[str, Any]):
...
def _write_success_contracts(
    ...
    output = root / "artifacts/model_improvement_v2/experiments/E09"
    ...
    eligible = [
        row for row in rows
        if row["source"] == "E09_TRAINED_CHALLENGER"
    ...
    decision = {
        "action": "PROMOTE_SGDM" if promoted else "RETAIN_E01_ADAMW",
        ...
        "selected_weight_decay": promoted["weight_decay"] if promoted else 1e-3,
    ...
    comparison = {
        "schema": "MODEL_IMPROVEMENT_V2_E09_SGDM_ABLATION-v1",
        "experiment_id": EXPERIMENT_ID,
        "primary_change": "SGDM_FAMILY_WITH_PREDECLARED_LR_AND_WEIGHT_DECAY",
        "primary_metric": "pooled_rmse_wh",
        "results": rows,
    ...
    atomic_write_bytes(output / "e09_execution_manifest.json", canonical_json_bytes(manifest))
    atomic_write_bytes(output / "e09_sgdm_ablation_comparison.json", canonical_json_bytes(comparison))


def run_official(root: Path | None = None) -> int:
    resolved = root or project_root()
    document = load_e09_config(resolved)
    preflight = validate_e09_document(document, resolved)
    context = build_e09_run_context(resolved, document)
...
def run_resume_stage_c(root: Path | None = None) -> int:
    ...
    document = load_e09_config(resolved)
    preflight = validate_e09_document(document, resolved)
    context, resume_audit = build_e09_resume_context(resolved, document)
    from course_work.rolling_origin.real_run import run_real_pipeline
    result = run_real_pipeline(context)
    if result.exit_code != 0:
        print(result.summary, file=sys.stderr)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="V2 E09 SGDM optimizer audit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.seed != SUPPORTED_SEED:
        print(f"ERROR: E09 screening seed must be {SUPPORTED_SEED}", file=sys.stderr)
        return 2
    if args.mode == "official" and not args.authorize_training:
        print("REFUSED: E09 official mode requires --authorize-training.", file=sys.stderr)
        return 3
    ...
    try:
        ...
        return 0
    except (E09PreflightError, PermissionError, ValueError) as exc:
        print(f"E09 {args.mode.upper()} FAIL: {exc}", file=sys.stderr)
        return 1
