"""E05 controlled residual gate OFF versus ON ablation."""
...
EXECUTION_TRACK = "MODEL_IMPROVEMENT_V2_E05"
REGISTRY_NAMESPACE = "V2_E05"
CONFIG_RELATIVE_PATH = Path("artifacts/model_improvement_v2/experiments/E05/e05_config_snapshot.json")
...
EXPECTED_CONFIG_CHANGES = ("model.residual_gate",)
EXPECTED_GATE_SPEC = {
    "input": "TRANSFORMER_POOLED_LAST_STEP_D64",
    "logits": "Linear(64,1)",
    "activation": "sigmoid",
    "formula": "y_hat = y_context + gate * delta_hat_raw",
}
GATE_INITIALIZATION_POLICY = "ZERO_WEIGHT_ZERO_BIAS_FIXED"
...
class E05PreflightError(RuntimeError):
    pass
...
def _read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise E05PreflightError(f"Cannot load {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise E05PreflightError(f"Expected JSON object: {path}")
    return value
...
def load_e05_config(root: Path) -> dict[str, Any]:
...
def _verify_artifacts(base: Path, expected: Mapping[str, Any], label: str) -> None:
    for name, checksum in expected.items():
        ...
        if not path.is_file() or _sha256(path) != checksum:
            raise E05PreflightError(f"{label} immutable artifact changed: {name}")
...
def _validate_local_control(root: Path, document: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if control.get("reuse_policy") != "CONTROL_REUSED_FROM_E03":
        raise E05PreflightError("E05 local control must be reused from E03")
    if (
        control.get("prediction_formulation") != PREDICTION_FORMULATION
        or control.get("prediction_head") != PREDICTION_HEAD
        or control.get("residual_gate") != CONTROL_GATE
    ):
        raise E05PreflightError("E05 local control must be residual LINEAR gate OFF")
    if control.get("test_status") != "NOT_ACCESSED":
        raise E05PreflightError("E03 local control violates Test firewall")
    ...
    if len(run_ids) != 6 or len(set(run_ids)) != 6 or not all("_E03_" in value for value in run_ids):
        raise E05PreflightError("E03 local-control run ledger is incomplete")
...
def _validate_global_baseline(root: Path, document: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if baseline.get("reuse_policy") != "GLOBAL_BASELINE_REUSED_FROM_E01":
        raise E05PreflightError("E05 global baseline must be reused from E01")
    if (
        baseline.get("prediction_formulation") != "DIRECT"
        or baseline.get("residual_gate") != CONTROL_GATE
        or baseline.get("test_status") != "NOT_ACCESSED"
    ):
        raise E05PreflightError("E01 global baseline contract mismatch")
    ...
    if observed != EXPECTED_GLOBAL_METRICS or baseline.get("pooled_metrics") != observed:
        raise E05PreflightError("E01 accepted global metrics mismatch")
    if comparison.get("target_population_match") is not True or comparison.get("test_status") != "NOT_ACCESSED":
        raise E05PreflightError("E01 global baseline is not canonical pre-Test evidence")
    execution = _read_json(e01_root / "e01_execution_manifest.json")
    if execution.get("run_ids") != baseline.get("run_ids"):
        raise E05PreflightError("E01 global-baseline run ledger mismatch")
    return {"status": "PASS", "metrics": observed}



def _validate_rejected_e04(root: Path, document: Mapping[str, Any]) -> dict[str, Any]:
    rejected = document.get("rejected_predecessor", {})
    if rejected.get("policy") != "NOT_CARRIED_FORWARD_NOT_A_CONTROL":
        raise E05PreflightError("E04 must not be carried forward into E05")
    e04_root = root / "artifacts/model_improvement_v2/experiments/E04"
    _verify_artifacts(e04_root, rejected.get("artifact_sha256", {}), "E04")
    comparison = _read_json(e04_root / "e04_head_ablation_comparison.json")
    challenger = comparison.get("global_comparison", {}).get("challenger_metrics", {})
    if challenger.get("rmse_wh") != 60.66433497520889:
        raise E05PreflightError("E04 rejected RMSE evidence mismatch")
    if comparison.get("global_comparison", {}).get("promotion_status") != "DO_NOT_PROMOTE_E04_GLOBAL":
        raise E05PreflightError("E04 is not recorded as rejected")
    if comparison.get("test_status") != "NOT_ACCESSED":
        raise E05PreflightError("E04 evidence violates Test firewall")
    if rejected.get("pooled_metrics") != challenger or rejected.get("promotion_status") != "DO_NOT_PROMOTE_E04_GLOBAL":
        raise E05PreflightError("E04 rejection snapshot mismatch")
    return {"status": "PASS", "rmse_wh": 60.66433497520889, "carried_forward": False}

def validate_e05_document(document: Mapping[str, Any], root: Path) -> dict[str, Any]:
    if document.get("experiment_id") != EXPERIMENT_ID or document.get("track") != "MODEL_IMPROVEMENT-v2":
        raise E05PreflightError("Wrong E05 identity/track")
    if document.get("objective") != "CONTROLLED_RESIDUAL_GATE_ABLATION":
        raise E05PreflightError("Wrong E05 objective")
    primary = document.get("primary_change", {})
    if primary != {"factor": "model.residual_gate", "control": CONTROL_GATE, "challenger": CHALLENGER_GATE}:
        raise E05PreflightError("E05 primary change must be residual_gate OFF -> ON")

    challenger = document.get("challenger", {})
    config = challenger.get("config")
    if not isinstance(config, dict):
        raise E05PreflightError("Missing E05 challenger config")
    ...
    if (
        model.get("prediction_formulation") != PREDICTION_FORMULATION
        or model.get("prediction_head", "LINEAR") != PREDICTION_HEAD
        or model.get("residual_gate") != CHALLENGER_GATE
    ):
        raise E05PreflightError("E05 must be residual + LINEAR + gate ON")
    if (model.get("d_model"), model.get("output_size"), model.get("dropout")) != (64, 1, 0.1):
        raise E05PreflightError("E05 locked model dimensions/dropout changed")
    if challenger.get("gate_spec") != EXPECTED_GATE_SPEC:
        raise E05PreflightError("E05 gate specification mismatch")
    if challenger.get("gate_initialization_policy") != GATE_INITIALIZATION_POLICY:
        raise E05PreflightError("E05 gate initialization policy mismatch")
    if challenger.get("gate_initialization") != EXPECTED_GATE_INITIALIZATION:
        raise E05PreflightError("E05 gate initialization values mismatch")
    if challenger.get("residual_scaling_contract") != (
        "delta_model=(y_next-y_context)/y_std; delta_raw=delta_model*y_std; "
        "y_hat=y_context+gate*delta_raw"
    ):
        raise E05PreflightError("E05 residual scaling contract mismatch")
    data = config.get("data", {})
    if data.get("feature_variant_id") != FEATURE_VARIANT or data.get("feature_count") != 33 or model.get("input_size") != 33:
        raise E05PreflightError("E05 FS2_TF1 feature contract mismatch")
    ...
    if tuple(challenger.get("feature_order", ())) != tuple(get_feature_list(FEATURE_VARIANT)):
        raise E05PreflightError("E05 feature order mismatch")
    ...
    if changed != EXPECTED_CONFIG_CHANGES:
        raise E05PreflightError(f"Unexpected E05 changed fields: {changed}")
    fingerprint = compute_config_fingerprint(config)
    if challenger.get("config_fingerprint") != fingerprint:
        raise E05PreflightError("E05 config fingerprint mismatch")
    if challenger.get("initialization_policy") != "FRESH_FROM_SEED_42_NO_CHECKPOINT_LOAD":
        raise E05PreflightError("E05 challenger must initialize from scratch")
    ...
    for key, expected in expected_orchestration.items():
        if orchestration.get(key) != expected:
            raise E05PreflightError(f"E05 orchestration mismatch: {key}")
    for key in ("artifact_root", "registry_root", "run_root"):
        value = orchestration.get(key)
        if not isinstance(value, str) or "/experiments/E05" not in value:
            raise E05PreflightError(f"E05-owned path required: {key}")
    ...
    if (selection.get("primary_metric") != "pooled_rolling_origin_rmse_wh"
        ...
        or selection.get("global_promotion_rule") != "e05_pooled_rmse_wh <= global_promotion_threshold_rmse_wh"
        or selection.get("do_not_promote_for_beating_local_control_only") is not True
        or selection.get("weighted_composite") is not False
        or selection.get("test_metrics_allowed") is not False):
        raise E05PreflightError("E05 global promotion policy mismatch")

    local_audit = _validate_local_control(root, document)
    global_audit = _validate_global_baseline(root, document)
    rejected_e04_audit = _validate_rejected_e04(root, document)
    return {
        "status": "PASS", "experiment_id": EXPERIMENT_ID,
        "candidate_id": CANDIDATE_ID, "prediction_formulation": PREDICTION_FORMULATION,
        "prediction_head": PREDICTION_HEAD, "control_gate": CONTROL_GATE,
        "challenger_gate": CHALLENGER_GATE, "gate_spec": dict(EXPECTED_GATE_SPEC),
        "changed_config_fields": list(changed), "one_primary_change": "PASS",
        "local_control_reuse": "CONTROL_REUSED_FROM_E03",
        "local_control_audit": local_audit, "global_baseline_audit": global_audit,
        "rejected_e04_audit": rejected_e04_audit,
        "fresh_initialization": True, "deterministic_gate_initialization": True,
        "gate_initialization": dict(EXPECTED_GATE_INITIALIZATION),
        "training_candidates": ["RESIDUAL_TO_PERSISTENCE_LINEAR_GATE_ON"],
...
def run_preflight(root: Path | None = None) -> dict[str, Any]:
    ...
    return validate_e05_document(load_e05_config(resolved), resolved)


def build_e05_candidate(document: Mapping[str, Any]):
    ...
    return CandidateSpec(
        ...
        source_phase="V2_E05_IMMUTABLE_SNAPSHOT",
        candidate_role="RESIDUAL_LINEAR_GATE_ON_CHALLENGER",
    )
...
def build_e05_run_context(root: Path, document: Mapping[str, Any]):
    ...
    validate_e05_document(document, root)
    candidate = build_e05_candidate(document)
    artifact_dir = root / "artifacts/model_improvement_v2/experiments/E05"
    cache: dict[str, Any] = {}
    def dataset_factory(candidate_spec, _fold):
        if candidate_spec.feature_variant_id != FEATURE_VARIANT:
            raise E05PreflightError("E05 factory accepts only FS2_TF1")
        ...
        if audit.test_rows_read or audit.test_target_ids_seen:
            raise PermissionError("E05 Test firewall failed")
        return dataset
...
def _write_success_contracts(root: Path, document: Mapping[str, Any], result, preflight: Mapping[str, Any]) -> None:
    ...
    output = root / "artifacts/model_improvement_v2/experiments/E05"
    observed = result.pooled_metrics_by_cid[CANDIDATE_ID]
    manifest = {
        "schema": "MODEL_IMPROVEMENT_V2_E05_EXECUTION-v1", "experiment_id": EXPERIMENT_ID,
    ...
    comparison = {
        "schema": "MODEL_IMPROVEMENT_V2_E05_GATE_ABLATION-v1", "experiment_id": EXPERIMENT_ID,
        "primary_change": "RESIDUAL_GATE", "prediction_formulation": PREDICTION_FORMULATION,
        "prediction_head": PREDICTION_HEAD,
        "gate_formula": "y_hat = y_context + gate * delta_hat_raw",
        "local_comparison": {
            "control": "E03_RESIDUAL_LINEAR_GATE_OFF", "challenger": "E05_RESIDUAL_LINEAR_GATE_ON",
        ...
        "global_comparison": {
            "baseline": "E01_DIRECT_FS2_TF1", "challenger": "E05_RESIDUAL_LINEAR_GATE_ON",
            ...
            "promotion_rule": "e05_pooled_rmse_wh <= promotion_threshold_rmse_wh",
            "promotion_status": "PROMOTE_E05_GLOBAL" if promoted else "DO_NOT_PROMOTE_E05_GLOBAL",
            "local_win_alone_is_sufficient": False,
        },
        "target_population_policy": "IDENTICAL_PHASE44_RO1_RO2_RO3",
        "selection_metric": "pooled_rmse_wh", "test_status": "NOT_ACCESSED",
    }
    atomic_write_bytes(output / "e05_execution_manifest.json", canonical_json_bytes(manifest))
    atomic_write_bytes(output / "e05_gate_ablation_comparison.json", canonical_json_bytes(comparison))


def run_official(root: Path | None = None) -> int:
    resolved = root or project_root()
    document = load_e05_config(resolved)
    preflight = validate_e05_document(document, resolved)
    context = build_e05_run_context(resolved, document)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="V2 E05 residual gate OFF vs ON ablation")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.seed != SUPPORTED_SEED:
        print(f"ERROR: E05 screening seed must be {SUPPORTED_SEED}", file=sys.stderr); return 2
    if args.mode == "official" and not args.authorize_training:
        print("REFUSED: E05 official mode requires --authorize-training.", file=sys.stderr); return 3
    ...
    except (E05PreflightError, PermissionError, ValueError) as exc:
        print(f"E05 {args.mode.upper()} FAIL: {exc}", file=sys.stderr); return 1
