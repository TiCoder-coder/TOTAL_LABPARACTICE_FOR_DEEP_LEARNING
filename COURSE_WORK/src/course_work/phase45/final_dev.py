...
def build_final_dev_population(
    ...
    # Parse boundaries for first_test_timestamp and split_version fingerprint
    ...
    with boundaries_path.open("r", newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            # fingerprint column gives split-specific checksum (we don't pin to it)

    if first_test_ts is None:
        raise RuntimeError("TEST split boundary missing in split_boundaries.csv")

    # Stream membership to keep memory low: only TRAIN + VALIDATION rows
    target_ids: list[str] = []
    ...
    # Hard-assert last_timestamp < first_test_timestamp (lexicographic ok for ISO-like ts)
    if not (last_ts < first_test_ts):
        raise RuntimeError(
            f"FINAL_DEV last timestamp {last_ts!r} not < first TEST timestamp {first_test_ts!r}"
        )

    # Deterministic fingerprint: SHA256 over sorted, normalized, joined target_ids
