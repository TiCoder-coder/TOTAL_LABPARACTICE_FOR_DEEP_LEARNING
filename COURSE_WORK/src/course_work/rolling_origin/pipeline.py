...
class PipelineConfig:
    ...
    # TASK 9 (corrective): the scientific max_epochs cap that any candidate
    # Stage A must use. The default of 50 matches the Phase 44 plan contract.
    # Official mode (enable_scientific_training=True, rehearsal=False) requires
    # scientific_max_epochs >= 50.
...
def _build_fold_metrics_rows(
    ...
    # Compute ranks (per fold, ascending RMSE)
...
def _build_pooled_metrics_rows(
        ...
        rows.append(
            {
                ...
                + sum(
                    1 for _ in []  # placeholder — populated below from pooled predictions
                ),
    ...
    # Compute pooled counts from candidate fold outer counts
    return rows, family_map
...
def _derive_predictions_payloads(
    *,
    ...
    for f in folds:
        # Resolve target_ids → rows in windowpop to read observed y_true.
...
def _assert_official_mode_guards(cfg: PipelineConfig) -> None:
    ...
    if cfg.rehearsal:
        # Rehearsal is allowed: it writes to staging and uses synthetic data.
        # But it must NOT enable scientific training.
        ...
        # Rehearsal may use any epoch cap, but cap must be <= scientific cap.
...
def run_pipeline(cfg: PipelineConfig) -> PipelineResult:
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # TASK 9: official mode hard guard. The synthetic pipeline below
        # NEVER calls train_stage_a()/engine.train()/refit_engine.refit().
        # For Phase 44 official we must reject this path until the real
        # orchestration (official_run.py or successor) replaces it.
        # ------------------------------------------------------------------
        ...
        # Build Stage A/B registration payloads (24 total)
        ...
        # Derive BEST epochs (deterministic for the run)
        best_epochs: dict[tuple[str, str], int] = {}
        # For stability: best_epoch comes from training; in this synthetic
        # path we derive it deterministically but capped by rehearsal_epoch_cap.
        for f in folds:
            for c in candidates:
                # Derive a deterministic seed-stable epoch count
        ...
        # Materialize outer predictions (per-target persistence).
        ...
        if "Appliances" not in windowpop_df.columns:
            # WINDOWPOP-v1 is metadata only. Reconstruct by joining through
            # raw_row_index using the validated FEATURES-v1 derived file.
            try:
        ...
        # Best epochs drive deterministic predictions for non-persistence
        # candidates (in this synthetic-friendly default).
        ...
        # For non-persistence model evaluations we need to override the
        # prediction payload (per-fold predictions for learned models).
        # In rehearsal we use the derived predictions as a stand-in for
        # the real Stage C outputs. In production this would come from the
        # real Stage C model + scaler bundle.
        for key, val in derived_predictions.items():
            per_fold_predictions[key] = val
        # Persistence (last-value) keeps its own prediction.

        # Pooled predictions DF and uniqueness assertions
        ...
        # Validate pooled population for each candidate
        for c in candidates:
        ...
        # Pooled metrics per candidate (recomputed from concatenated residuals)
        ...
        # Transformer ranking (filter for Transformer family only)
        family_map = {c.candidate_id: c.model_family for c in candidates}
        ...
        # Augment ranking_rows with all_csv fields (rank_complete)
        ...
        # Find recommended transformer (top of ranking)
        ...
        # Findings (use codes from plan §181)
        ...
        # Tests table (record preflight + rehearsal smoke)
        tests_rows = [
            {
        ...
        # Discrepancies (empty unless we encountered any)
        discrepancies = {
            "phase44_version": ROBASE_VERSION,
            "discrepancies": [],
            "total": 0,
        ...
        # Pairwise effects (pooled-level only for brevity)
        pairwise_rows = []
        ...
        # Common target audit (per fold, per candidate: assert same target ids across candidates)
        common_target_rows = []
        for f in folds:
            ...
            for c in candidates:
                ...
                # y_true equality (assertion)
                yt_base = np.asarray(
                    per_fold_predictions[(c.candidate_id, str(f.fold_id))]["y_true_wh"]
                )
                same_y = (
                    all(v == v for v in yt_base)  # not nan, otherwise fine
                )
        ...
        # Population audit rows (for O44.6 we use the probe-generated rows
        # but normalized to the writer schema)
        ...
        # Init / sample order / runtime / gradient diagnostics
        ...
        # Pooled metrics rows (use pooled_metrics_by_cid + pooled count from
        # pooled_predictions_df per (candidate_id))
        pooled_metrics_rows = []
        for c in candidates:
            pm = pooled_metrics_by_cid[c.candidate_id]
            pop_count = int(
        ...
        # Scaler-fit audit rows (passthrough from probe rows)
        ...
        # Write all artifacts through the centralized writer
        artifact_paths = _write_all(
            ...
            signoff={
                ...
                # TASK 9 (corrective): all_candidates_complete must NOT be True
                # when no candidate actually trained. In the synthetic path,
                # the candidate has no real Stage A/B/C runs on disk.
                ...
                # TASK 9: nested_epoch_selection_verified is False because
                # the synthetic path does not actually perform epoch selection.
                ...
                "warnings": [],
                # TASK 9 (corrective): overall_status must be derived from
                # real gates, NOT hardcoded PREPARED.
                # The synthetic mock path below produces PREPARED as a marker
                # that this is a rehearsal/mock; official mode is rejected
                # by the guard above.
                "overall_status": "PREPARED",
                "created_at": _now_iso(),
            },
