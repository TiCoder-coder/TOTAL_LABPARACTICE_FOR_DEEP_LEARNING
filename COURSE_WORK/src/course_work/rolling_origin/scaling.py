...
class FoldLocalScalerBundle:
    ...
    fold_id: str              # e.g. "RO1"
    candidate_id: str
    target_scaling_option: str  # "YS0" or "YS1"
    ...
    # X scaler — list of per-column means and stds (numpy float32)
    ...
    # Y scaler — single mean/std for YS1; None for YS0
    ...
    def as_dict(self) -> dict:
        ...
        # ── YS1 contract alignment ────────────────────────────────────────
        # The canonical `transform_target` / `inverse_transform_target`
        # functions in `course_work.data.scaling` require the bundle dict
        # to expose:
        #   - `option`            (e.g. "YS1")  [aliased from target_scaling_option]
        #   - `scaler`            fitted sklearn-like scaler with
        #                         .transform / .inverse_transform
        #   - `scaling_version`   in {SCALING-v1, FINAL_SCALING-v1}
        # The fold-local bundle stores per-column means/stds lists; we
        # build a fitted `StandardScaler` from these so the canonical
        # contract is satisfied (without weakening global scaling
        # validation).
        ...
        if self.target_scaling_option == "YS1":
            ...
            # YS0 is identity — provide a no-op scaler with .transform
...
class ScalerFitAudit:
    ...
    fit_start_timestamp: str  # ISO from windowpop
    fit_end_timestamp: str

    allowed_target_history_region: str
    outer_eval_rows_used: int  # must be 0
    test_rows_used: int        # must be 0
...
def record_scaler_fit_audit(
    *,
    ...
    # Normalize: windowpop_df["target_id"] can be int or str. We build a
    # set of canonical (string) ids so we can match either form.
    def _norm(t):
        return str(t)
    ...
    # If windowpop stores IDs as ints (e.g. raw row indices), we need the
    # int form too. We discover the format empirically: read the first
    # windowpop target_id and see whether it parses as int.
    ...
    if wp_is_int:
        # Convert fit/outer/test ID strings to their int form for matching.
        def _to_int_form(s):
    ...
    # Raw-row counts (per windowpop row, not per target_id)
    fit_raw_rows = int(windowpop_df["target_id"].isin(fit_tids).sum())
    fit_windowpop = windowpop_df[windowpop_df["target_id"].isin(fit_tids)]
    # Outer eval / test rows are those in windowpop whose target_id is in
    # outer_eval_tids / test_tids. The audit asserts these rows are NOT
    # also in the FIT region. This is detected by intersecting fit_tids
    # with outer_eval_tids: if any overlap, the scaler was fit on data
    # that includes outer_eval rows, which is a leakage violation.
