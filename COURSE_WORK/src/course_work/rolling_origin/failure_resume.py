...
def simulate_failure(scenario: FailureScenario) -> FailureAudit:
    if scenario == FailureScenario.DURING_STAGE_A:
        return _audit_template(
            ...
            new_run_id_required=True,  # NEW STAGE_A_RUN_ID
            orphan_records_left=[],  # registry.fail_run() flips status to FAILED
            notes=(
    ...
    if scenario == FailureScenario.AFTER_STAGE_A_BEFORE_B:
        return _audit_template(
            ...
            new_run_id_required=False,  # resume — same run IDs preserved
            orphan_records_left=[],
            notes=(
