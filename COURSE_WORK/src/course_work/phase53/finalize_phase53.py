...
def run_phase53_finalize(project_root: Path, orchestrator_summary: dict | None = None) -> dict:
    ...
    # Load raw file SHAs from handoff or summary
    ...
    case_order_sha = orchestrator_summary["raw_source_verification"]["42"]["observed_sha256"]  # not the order, used as placeholder
    case_order_sha = ""
    position_map_sha = ""
        ...
        for py_fp in sorted(phase53_dir.glob("*.py")):
            ...
            for pat in _FORBIDDEN_PATTERNS:
                for m in re.finditer(pat, stripped, flags=re.MULTILINE):
                    ...
                    # Skip the orchestrator's own documentation/comment-like usage
                    # We only flag code that calls these functions.
