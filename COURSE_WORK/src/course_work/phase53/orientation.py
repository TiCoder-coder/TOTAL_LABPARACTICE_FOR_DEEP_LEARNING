...
def synthetic_orientation_test(L: int = LOOKBACK) -> dict:
    ...
    # Verify no transpose: if we plot M, expected x=s, y=q
    # M[q,s] ⇒ display row = q (y), display column = s (x)
    return {
        "matrix_shape": [L, L],
        "query_idx": q_marker,
        "source_idx": s_marker,
...
def real_source_orientation_test(
    ...
    M = arr[case_row_idx0, layer_idx0, head_idx0, :, :]  # [L, L]
    L = M.shape[0]
    # Choose a few fixed q,s coords and read values
    coords = [(0, 0), (0, L - 1), (L - 1, 0), (L - 1, L - 1), (1, 3), (3, 0)]
    seen = []
    for q, s in coords:
        # In our contract: M[q,s] ⇒ display row=q (y), display column=s (x)
