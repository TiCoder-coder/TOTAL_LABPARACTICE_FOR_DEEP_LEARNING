...
def cliffs_delta(x: np.ndarray, y: np.ndarray) -> float:
    ...
    if n_x == 0 or n_y == 0:
        return 0.0
    # Fast computation via combined sort
    ...
    while i < n:
        ...
        i = j
    # Sum ranks for y elements (last n_y elements of concatenated array)
    # Convention: x at indices [0..n_x), y at indices [n_x..n_x+n_y)
    R_y = ranks[n_x:].sum()
    # Cliff's delta = (2*R_y - n_y*(n + 1)) / (n_x * n_y)
...
# ---------------------------------------------------------------------------
# Spearman correlation (constant-input safe)
# ---------------------------------------------------------------------------
