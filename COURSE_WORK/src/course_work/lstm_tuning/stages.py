...
class PlannedCandidate:
    ...
    value: Any
    config: dict[str, Any]
    source_type: str  # REUSED_REFERENCE | FRESH
...
class StageExecutor:
    ...
    def plan_stage(self, stage: str) -> PlannedStage:
        ...
        for option, value in candidate_options.items():
            ...
            # The reference candidate is considered "reused" (not counted as
            # a fresh scientific run) even when no concrete run_id exists yet.
            # The first LT1 reference may still need a single LSTM_T0 training,
            # but that single fresh reference run amortises across all stages
            # and is counted separately as one fresh run, not 5.
