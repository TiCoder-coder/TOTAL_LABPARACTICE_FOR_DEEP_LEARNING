...
# Make sibling modules accessible when run as a script
...
def run_phase56(project_root: Path | str = ".") -> dict:
    ...
    # 2. Build cohort assignment (FROZEN before attention join)
    ...
    # 3. Save cohort assignment + audit
    assignment_cols = [
    ...
    # 7. Error-decile metric summary + per-decile profile (layer head-mean)
    ...
    # Layer head-mean profiles for HIGH/LOW
    ...
    # Layer head-mean UNDER/OVER profiles
    ...
    # 11. Layer head-mean: per-target metrics + association + high_low + shared + cross-seed
    ...
    # Patch profile_jsd and profile_wasserstein_minutes for layer_high_low
    ...
    # SHARED cohort layer head-mean profiles (for fig 11)
    ...
    # 14. Worst-case context (W2 shared ranks 1-5) — using layer head-mean metrics
