...
class CohortAssignmentResult:
    ...
    assignment_df: pd.DataFrame  # long form: (seed, target_id, ...) rows = 3*N_TEST
    shared_hardness_df: pd.DataFrame  # (target_id, shared_hardness) N_TEST rows
...
def assign_seed_cohorts(seed_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Sort by absolute_error_wh ASC, then timestamp ASC, target_id ASC
    ...
    # Decile assignment: decile = 1 + floor(10*r/N), capped at 10
    deciles = []
    for r in range(n):
        d = 1 + (10 * r) // n
        if d > 10:
...
def build_cohort_assignment(sources: FrozenSources56) -> CohortAssignmentResult:
    ...
    """
    # Build seed-specific df
    res_df = pd.DataFrame(sources.residual_long)
    # Filter to Test seeds
    ...
    # Build shared cohort assignment (on shared_hardness ordering)
    ...
    # SHA256 of assignment (deterministic, frozen)
    ...
    # SHA256 of shared hardness file
