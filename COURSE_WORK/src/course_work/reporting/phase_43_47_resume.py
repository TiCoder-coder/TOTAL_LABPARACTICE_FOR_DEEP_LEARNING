...
# ---------------------------------------------------------------------------
# Shared CSS — single style block, included once at the top of every renderer
# ---------------------------------------------------------------------------
...
def render_phase_43_lstm_tuning_resume(project_root: Path | None = None) -> HTML:
    ...
    # Tuning lineage table — 5 rows for LT1..LT5
...
def render_phase_44_rolling_origin_resume(project_root: Path | None = None) -> HTML:
    ...
    # Pooled metrics: build cards for each model
    cards = []
    for row in pooled:
    ...
    # Inner best epochs table for recommended Transformer
...
def render_phase_46_three_seed_final_runs_resume(project_root: Path | None = None) -> HTML:
    ...
    # Seed cards (final-dev diagnostic, NOT held-out test)
    seed_42 = {
...
def render_phase_47_final_test_evaluation_resume(project_root: Path | None = None) -> HTML:
    ...
    # Comparison: Transformer vs Persistence
    comp_rows = [
        [
            "MAE (Wh)",
    ...
    # Scientific conclusion (deterministic from canonical signoff)
