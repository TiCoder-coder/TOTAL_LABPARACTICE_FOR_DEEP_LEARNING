...
# Explicit phase roles; analysis phases do not introduce winner selection.
SECTIONS = {
...
def apply_layout(html, phase, root):
        ...
        if is_overview or is_signoff:
            for label, value in _pairs(table):
                ...
                if re.search(r'phase.*status|phase 59 scientific conclusion', label, re.I):
                    continue  # Status is sourced once from the signoff, below.
                if is_signoff:
            ...
            # Renderers that emit a dedicated conditions table (e.g. Phase 44
            # "Rolling-origin conditions") should populate the canonical
            # conditions bucket rather than the result bucket. Each row is
            # classified by its label; non-condition keywords remain in
            # result_pairs so they are still surfaced.
            for label, value in _pairs(table):
                if _condition(label):
            ...
            # Empty source tables must remain visibly unavailable, not imply success.
    ...
    # Preserve every unique row once; results take priority over summary copies.
    seen = set()
    def unique(rows):
        output = []
    ...
    for image in re.findall(r'<img\b[^>]*>', html):
        # The image payload and alt text are retained byte-for-byte.
