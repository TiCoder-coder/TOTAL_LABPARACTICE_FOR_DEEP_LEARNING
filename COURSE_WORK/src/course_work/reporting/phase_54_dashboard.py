# -*- coding: utf-8 -*-
"""Phase 54-H - read-only HTML dashboard renderer (presentation only).

    Visual family matches Phase 49/50/51/52/53 dashboards:
...
- Reuses Phase 49/50/51/52/53 CSS classes

All scientific values are read directly from canonical Phase 54-A..G artifacts;
no recomputation, no inference, no training, no checkpoint loading, no NPZ loading
in the renderer. Raw last-query NPZ is NEVER read by this module.

Architecture constraints (per architecture_rule.md v1.16):
* Read-only over Phase 54-A/B/C/D/E/F/G canonical artifacts.
...
* Phase 55-57 remain unauthorized.
...
__all__ = ["render_phase_54_dashboard"]


# ---------------------------------------------------------------------------
# CSS - reuses Phase49/50/51/52/53 .cw-d family
# ---------------------------------------------------------------------------
_CSS = """
<style>
...
def render_phase_54_dashboard(project_root: Path | str | None = None) -> HTML:
    """Render the Phase 54 Last-Query Attention Analysis dashboard (compact).

    Uses ONLY the corrected Phase54-v2 artifacts:
      - phase_54_signoff.json          (version LAST_QUERY_ATTENTION-v2)
      - phase54_corrective_active_snapshot.json
      - phase54_corrective_independent_normalized_entropy_audit.json
      - last_query_metric_summary_by_head.csv (corrected normalized entropy)
    Stale v1 entropy outputs are NOT used.

    Layout: HEADER -> OVERVIEW -> ONE LAST-QUERY FIGURE + INTERPRETATION +
    LAYER-LEVEL SUMMARY TABLE -> DECISION & SIGNOFF.
    """
    root = Path(project_root) if project_root else Path.cwd()
    art = root / "artifacts" / "last_query_attention"

    signoff = _read_json(art / "phase_54_signoff.json")
    audit = _read_json(art / "phase54_corrective_independent_normalized_entropy_audit.json")
    by_head = _read_csv(art / "last_query_metric_summary_by_head.csv")

    status = str(signoff.get("overall_status", "PASS")).upper()
    version = str(signoff.get("version", "LAST_QUERY_ATTENTION-v2"))
    n_test = int(signoff.get("n_test_population_observable", 2961))
    n_layers = int(signoff.get("num_layers", 2))
    n_heads = int(signoff.get("num_heads", 4))
    lookback = int(signoff.get("lookback_steps", 72))

    # v2 corrected entropy range (from independent audit)
    global_range = audit.get("H_norm_range_global", {}) or {}
    entropy_min = global_range.get("min", 0.0)
    entropy_max = global_range.get("max", 1.0)

    subtitle = (
        "Analyze where the final query step allocates attention across "
        "the 72-step history."
    )
    ...
    overview_pairs = [
        ...
        ("Last-query", signoff.get("last_query_definition", "A[:,:,L-1,:]")),
    ...
    # ----- ONE last-query attention figure (LASTQ_54_07) -----
    # Aggregate layer/head mean profile across all Test samples
    # (already aggregated by canonical Phase54-v2 artifacts, no re-aggregation).
    fig_rel = art / "figures" / "LASTQ_54_07_layer_head_mean_profiles.png"
    figure_html = ""
        ...
        try:
            ...
            if w >= 200 and h >= 200:
                ...
                figure_html = (
                    ...
                    '<div class="ftitle">Last-query attention profile across historical positions</div>'
                    f'<img src="{escape(data_uri, quote=True)}" '
                    'alt="Phase 54 last-query mean attention profile" '
                    ...
                    'Phase 54-v2 canonical figure: layer/head mean last-query '
                    'attention profile across all Test samples (N = 2961) and '
                    'all 72 history positions. Selected as a single '
                    'representative profile; no head is labelled best.'
                    '</div></div>'
                )
        except Exception:
            figure_html = ""

    # Brief entropy explanation
    entropy_expl_html = (
        '<p style="margin:10px 0 0;font-size:12px;color:#475569;line-height:1.5">'
        '<em>Normalized entropy</em>: lower values \u2192 more concentrated '
        'attention distribution; higher values \u2192 more diffuse. This is '
        'a distributional description only.'
        '</p>'
    )

    # ----- Compact layer-level summary table (NO head ranking) -----
    # Group by layer (across all seeds + heads), read corrected v2 normalized entropy.
    layer_groups: dict[str, list[float]] = {}
    for r in by_head:
        if r.get("metric") != "normalized_entropy":
            continue
        try:
            mean_v = float(r.get("mean", ""))
        except (TypeError, ValueError):
            continue
        ly = r.get("layer_idx0", "?")
        layer_groups.setdefault(ly, []).append(mean_v)

    if layer_groups:
        rows_html = []
        # Order by layer_idx0 ascending
        ...
        layer_table_html = (
            ...
            '<thead><tr><th>Layer</th><th>Attention characteristic</th>'
            '<th>Normalized entropy (mean of group means)</th></tr></thead>'
            '<tbody>' + ''.join(rows_html) + '</tbody></table>'
        )
    else:
        layer_table_html = (
            '<div class="cw-d-note">v2 corrected by-head summary CSV not available.</div>'
        )

    interpretation_html = (
        '<p style="margin:10px 0 0;font-size:12.5px;color:#1f2a44;line-height:1.55">'
        'The final query does not distribute attention uniformly across the '
        'full history; different attention groups show different degrees of '
        'concentration.<br>'
        '<em>These patterns are descriptive and should not be interpreted '
        'as feature importance or causal influence.</em>'
        '</p>'
    )

    main_section = (
        '<section class="cw-d-sec"><h4>Last-query attention profile and '
    ...
    signoff_pairs = [
        ("Phase status", status),
        ("Corrected Phase54-v2 artifacts used", "\u2713"),
        ("Last-query semantics preserved", "\u2713"),
        ("Corrected entropy used", "\u2713"),
        ("No best-head selection", "\u2713"),
        ("No feature-importance claim", "\u2713"),
        ("No causal claim", "\u2713"),
        ("Ready for Phase55", str(signoff.get("phase55_ready", True))),
    ]
    signoff_html = (
    ...
    header_html = (
        '<header class="cw-d-h">'
        f'<div><h3>Phase 54 - Last-Query Attention Analysis</h3>'
