...
# ---------------------------------------------------------------------------
# CSS - reuses Phase 49-55 .cw-d family
# ---------------------------------------------------------------------------
_CSS = """
<style>
...
def render_phase_56_dashboard(project_root: Path | str | None = None) -> HTML:
    ...
    # Cohort sizes are derived from cohort_rule = RANK_BASED_20_60_20
    # n_low = round(0.20 * n_test) = 592, n_high = 592, n_mid = 1777 per seed
    n_low_per_seed = 592
    n_mid_per_seed = 1777
    n_high_per_seed = 592
    ...
    # ----- ONE LOW-vs-HIGH canonical figure -----
    # ERRORATTN_56_11 - shared targets LOW vs HIGH attention profile
    # (paired target set, no head labelled best, descriptive only).
        ...
        try:
            ...
            if w >= 200 and h >= 200:
                ...
                figure_html = (
                    ...
                    'N_high = 592). Descriptive attention comparison only; '
                    'no head is labelled best.'
                    '</div></div>'
                )
        except Exception:
            figure_html = ""

    # ----- Compact LOW/MID/HIGH summary table -----
    # Compute cohort-level normalized entropy means using canonical v2 artifacts
    # (priority: high_low file for LOW/HIGH direct; deciles 1-2 / 3-8 / 9-10 for LOW/MID/HIGH).
