...
# Path to FINAL_SCALING-v1 registry
...
class Phase47ScalerWrapper:
    ...
    def transform_only(self, data: Any) -> Any:
        ...
        # The wrapper stores the outer dict: {"scaler": StandardScaler, "pass_through_feature_order": [...], ...}
        ...
        if isinstance(outer_dict, dict):
            ...
            n_total = outer_dict.get("n_features_in_")  # might be 33
        else:
            # Plain StandardScaler — no outer dict, no pass-through
        ...
        # Number of pass-through features
        n_pass = len(pass_through_order)
        # Number of scaled features (inferred from StandardScaler's n_features_in_ or mean_)
        ...
        if needs_split:
            # Full 33-feature input: split scaled (first n_scaled) + pass-through (last n_pass)
            ...
            # Already the scaled subset only
            result = inner_scaler.transform(data)
        else:
            # Fallback: try direct transform
            result = inner_scaler.transform(data)
...
def load_final_scaling_v1_x_scaler(
    ...
    # D-05 FIX: The joblib file contains a dict wrapping the StandardScaler:
    #   {"scaler": StandardScaler, "pass_through_feature_order": [...], "n_features_in_": 33, ...}
    #
    # We wrap the OUTER DICT (not the inner StandardScaler), so that transform_only()
    # can access pass_through_feature_order and the total feature count (33).
    # The actual StandardScaler is extracted only inside transform_only().
    if isinstance(scaler, dict):
        ...
        # Keep the outer dict — we'll extract the inner scaler inside transform_only()
        pass  # scaler stays as the dict
    elif hasattr(scaler, "_scaler"):
        # Already a Phase47ScalerWrapper — extract inner
        ...
        # Plain StandardScaler — no pass-through to handle
        pass
...
def load_final_scaling_v1_y_scaler(
    ...
    # YS0 is identity — no artifact needed
    ...
    # D-06 FIX: Y scaler joblib also contains a dict wrapping the StandardScaler.
    # Extract the inner StandardScaler for proper transform/inverse_transform.
    if isinstance(scaler, dict):
