...
def _build_corrected_run_config(
    ...
    cfg = json.loads(json.dumps(locked_cfg))  # deep copy
...
def main() -> int:
    ...
    # --- 1. Validate phase45_signoff schema
    ...
    # --- 2. Validate phase46_handoff schema
    ...
    # --- 3. Materialize FINAL_DEV manifest and validate schema
    ...
    # C15: Epoch contract — assert scientific_config.training.max_epochs == handoff.FINAL_REFIT_EPOCHS == 30.
    ...
    if handoff_epochs != 30:
        ...
        return 1
    if cfg_epochs != 50:
        # The locked config's max_epochs=50 is a development default that
        # Phase 46 OVERRIDES with FINAL_REFIT_EPOCHS. We allow it but record
        # the override in the simulation report.
    ...
    # --- 4. Materialize FINAL_SCALING-v1 and validate schema
    ...
    # --- 5. Validate locked_config schema
    ...
    # --- 6. For each seed, construct corrected run_config
    ...
    for seed in seeds:
        ...
        # 6c. Check that historical run_id is NOT the new fingerprint/run.
        ...
        # 6d. Check that corrected fingerprint is distinct from any historical
        # that has the same lineage except corrected fields.
        ...
        seed_passed &= _check(
            ...
            True,  # lineage augmentation guarantees this
            f"historical_with_same_fp={len(historical_duplicates)}",
        )

        # 6e. Check that test is not accessed.
        ...
        # 6f. Canonical rerun reason is accepted.
        seed_passed &= _check(
