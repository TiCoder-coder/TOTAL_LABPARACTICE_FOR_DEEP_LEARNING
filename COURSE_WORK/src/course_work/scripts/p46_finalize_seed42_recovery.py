...
# ---------------------------------------------------------------------------
# Path setup (mirror phase46_three_seed_runs.py)
# ---------------------------------------------------------------------------

# Bootstrap ROOT via the canonical helper.
...
# Canonical locked identities from Phase 45 (hard-coded from the locked
# artifacts to keep this helper independent of any other Phase 46 logic).
...
# Source run_id (RUN 0183) is fixed — this helper ONLY finalizes Seed 42
# from this specific interrupted pre-canonical training run.
...
def _verify_locked_identities(
    ...
    # Phase 46 handoff does not always carry a top-level `seeds` field; the
    # canonical seed set is recorded in Phase 45 signoff.seed_list.
    ...
    # X/Y FINAL_SCALING-v1 SHA verification is done in _verify_run_0183_evidence
    # from the on-disk joblib files; we do not require a separate manifest here.
...
def _archive_legacy_metadata(
    ...
    # Build relative paths against ROOT for portability, but fall back to
    # absolute paths when the source/seed42_dir lives outside ROOT (test
...
def _verify_run_0183_evidence() -> dict[str, Any]:
    ...
    # Deterministic mode (required for persistence-only equivalence)
    ...
    # Training history gates (epoch 30 must be the last row and is_best=True)
    ...
    # FINAL_REFIT semantics: train_rmse == validation_rmse for all epochs
    ...
    # Checkpoint binary verification (loadable + epoch=30)
...
def _build_final_refit_envelope(
    ...
    # The persisted FINAL_REFIT .pt will wrap the original payload (which
    # already contains model_state_dict + best_epoch + best_validation_rmse_wh
    # + criterion_config + checkpoint_metadata) with all canonical provenance
    # fields. We do NOT change model_state_dict — only ADD metadata.
...
def finalize_seed42(
    *,
    ...
    """
    # Install training-attempt guard
    _training_attempt_guard()

    # Load upstream locked artifacts
    ...
    # Verify RUN 0183 evidence
    evidence = _verify_run_0183_evidence()

    # Archive legacy metadata (if any) before overwriting
    ...
    # Build the FINAL_REFIT envelope
    ...
    # Persist .pt (with envelope)
    import torch

    # Write to a temp file then atomic-rename to avoid leaving partial
    # checkpoints on disk if anything goes wrong.
    tmp_pt = final_pt_path.with_suffix(final_pt_path.suffix + ".tmp")
    torch.save(persisted_payload, tmp_pt)
    tmp_pt.replace(final_pt_path)

    # Verify model_state_sha256 matches the on-disk file
...
def main() -> int:
    ...
    try:
        if args.dry_run:
            # Verify gates only; do not write anything.
            _training_attempt_guard()
            p45_signoff = _read_json(PHASE_45_SIGNOFF)
            p46_handoff = _read_json(PHASE_46_HANDOFF)
            final_dev_manifest = _read_json(FINAL_DEV_MANIFEST)
