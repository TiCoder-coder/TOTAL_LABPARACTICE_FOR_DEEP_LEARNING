...
def main() -> int:
    ...
    # 1) Phase44 PASS / approved_for_phase45 / test_status NOT_ACCESSED
    ...
    # 4-5) Epochs present + median rule
    ...
    # 7) Scaler contract excludes Test
    ...
    # 9) Recipe: no validation / no early stopping / FINAL_REFIT
    recipe = build_recipe(
        candidate.config,
    ...
    # 10) Run matrix: exactly 3 planned runs, same epochs
    ...
    # 13) Zero training / validation paths
    # NOTE: The dev-time candidate.config has early_stopping_enabled=True because that
    # was its Phase42 specification (used during Stage A inner validation). The Phase46
    # FINAL_REFIT_MODE-v1 recipe overrides this to early_stopping=False (verified above).
    # The pre-lock contract freezes the RECIPE, not the source config's dev fields.
    if recipe_dict["early_stopping"] is not False:
        fails.append("Phase46 FINAL_REFIT recipe must have early_stopping=False")

    # 14) WB0 primary / no amendment
