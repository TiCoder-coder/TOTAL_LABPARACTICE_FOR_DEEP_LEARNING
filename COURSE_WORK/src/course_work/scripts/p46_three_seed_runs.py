...
# Historical checkpoint archive (preserved evidence of invalidated Phase 46
# runs).  Used to detect and quarantine stale copies that may have been
# re-materialized during prior failed attempts.
...
def _quarantine_stale_active_checkpoints() -> dict[str, Any]:
    ...
    if not HISTORICAL_ARCHIVE_MANIFEST.exists():
        # No archive yet; nothing to quarantine against.
    ...
    # Walk active seed_<N>/ directories.
    for seed_dir in sorted(CHECKPOINT_DIR.glob("seed_*")):
        ...
        if active_sha in historical_shas:
            ...
            # Quarantine: move file to QUARANTINE_DIR/<historical_rid>/.
...
# Phase 46 corrects: FINAL_DEV + FINAL_SCALING
...
def materialize_seed_runs_from_existing_artifacts() -> dict[str, Any]:
    ...
    for run_dir in sorted(runs_root.glob("RUN_TR_FSD_*")):
        ...
        # entry. Phase 46 corrected runs are one-time completions of a recovery or
        # a corrected training pass — they are NOT candidates for reuse in subsequent
        # Phase 46 invocations because the corrected config_fingerprint is already
        # different from the historical one. Reusing a corrected Phase 46 run would
        # bypass the deliberate cache_valid check that prevents stale entries.
        historical_predecessor = config.get("lineage", {}).get("historical_invalidated_predecessor_run_id", "")
        if not historical_predecessor:
            continue  # skip — corrected Phase 46 runs are not reusable cache candidates
...
# Canonical Phase 45 → 46 handoff model identifier fields (in resolution order).
# The canonical field is "candidate_id" (Phase 45 Phase 46 handoff contract).
# No other field name is authoritative; missing fields are errors, not fallbacks.
...
# Mapping: seed → historical invalidated Phase 46 run ID (predecessor).
# Used to record the corrected rerun's parent_run_id and lineage.
...
# ---------------------------------------------------------------------------
# PRE-TRAIN SCHEMA CONTRACTS
# ---------------------------------------------------------------------------
# These dicts declare the fields each producer MUST guarantee BEFORE any
# run registration.  ``_validate_*()`` helpers raise informative
# ``PreTrainSchemaError`` exceptions with the actual missing/extraneous
# keys, so a key mismatch is caught immediately, not at registration
# or training time.
...
# Canonical Phase 46 handoff (phase46_three_seed_handoff.json) keys the
# driver depends on.  Phase 45 writes these fields; the driver MUST
# resolve each via the canonical field name.
...
# Canonical Phase 45 signoff (phase_45_signoff.json) keys.
_PHASE45_SIGNOFF_REQUIRED_FIELDS = (
    "status",
    "final_lock_sha256",
)

# Canonical FINAL_SCALING-v1 result (materialize_final_scaling_v1) keys.
...
# Canonical FINAL_DEV manifest attributes (FinalDevRegionManifest).
...
# Canonical locked scientific config (final_model_scientific_config.json) keys.
# Note: ``training.seed`` and ``training.final_refit_mode`` are injected by the
# driver when constructing each per-seed run_config; they are NOT required in
# the locked scientific config itself.
...
def validate_phase46_handoff_schema(phase46_handoff: dict[str, Any]) -> None:
    ...
    # scientific_config must itself satisfy the locked-config schema.
...
def _build_phase47_release(
    ...
    """
    # Phase 47 release MUST NEVER reference historical INVALIDATED run IDs.
...
def validate_phase_46_preflight(
    ...
    # Gate 3: Lock hashes match
    p45_lock_sha = p45_signoff.get("final_lock_sha256", "MISSING")
    # Prefer final_lock_sha256 over config_fingerprint for cross-phase identity.
    ...
    # Gate 6: FINAL_SCALING materialization
    try:
    ...
    # Gate 7: Feature order matches lock
    ...
    # Gate 9: Epoch contract — read from Phase 45 handoff FINAL_REFIT_EPOCHS
    ...
    # Gate 10: FINAL_REFIT engine semantics
    ...
    # Gate 13: Scaler contract verification
    try:
...
def _main_inner(DRY_RUN: bool) -> None:
    ...
    # PRE-TRAIN SCHEMA VALIDATION: catch field mismatches before any
    # run registration or training step.  Raises PreTrainSchemaError.
    try:
        validate_phase45_signoff_schema(p45_signoff)
        validate_phase46_handoff_schema(p46_handoff)
        # validate_phase46_handoff_schema recursively validates scientific_config
    ...
    # CORRECTED: read FINAL_REFIT_EPOCHS exclusively from handoff.
    # Do NOT fall back to locked_cfg["training"]["max_epochs"] (=50, a development default).
    # Do NOT fall back to 50 as a default.
    ...
    # =================================================================
    # PREFLIGHT GATES (runs for both DRY_RUN and SCIENTIFIC)
    # =================================================================
    ...
    if DRY_RUN:
        ...
        # PRE-TRAIN ACTIVE-AREA CLEANUP — also runs in DRY_RUN so the
        # cleanup is verified before any scientific run is attempted.
    ...
    # =================================================================
    # SCIENTIFIC TRAINING PATH (only when DRY_RUN=False)
    # =================================================================
    print("\n[SCIENTIFIC] Preflight passed. Proceeding with Phase 46 training.")

    # PRE-TRAIN ACTIVE-AREA CLEANUP — defensive double-check (DRY_RUN branch
    # already exercised this; this guards against stale restoration between
    # dry-run and run).
    cleanup_audit = _quarantine_stale_active_checkpoints()
    if cleanup_audit.get("quarantined"):
        print(
    ...
    # Materialize FINAL_DEV and FINAL_SCALING
    final_dev_manifest = materialize_final_dev_region(
        project_root=ROOT,
    ...
    # PRE-TRAIN SCHEMA VALIDATION on the freshly materialized objects.
    try:
    ...
    # Load FINAL_SCALING-v1 X scaler bundle
    x_scaler_bundle = load_final_dev_x_scaler(variant_id, ROOT)

    # Load FINAL_SCALING-v1 Y scaler (YS1) or None
    if target_option == "YS1":
        target_scaler_bundle = load_final_dev_target_scaler(ROOT)
    else:
        target_scaler_bundle = None

    # Build FINAL_DEV dataset using the shared canonical feature timeline.
    # Uses FINAL_SCALING-v1 X scaler (via the registry path checked above).
    ...
    # Build DataLoader for FINAL_DEV (TRAIN+VALIDATION)
    ...
    # CLEAN: Remove stale historical invalidated entries that were restored during
    # the previous failed attempt (Sep 1 18:35). These are historical bytes that
    # must not satisfy corrected Phase 46 reuse eligibility.
    stale_entries = {
        str(seed): entry
    ...
    # NB: active-checkpoint quarantine (against historical archive SHAs)
    # happens earlier in main() under the SCIENTIFIC header so it also runs
    # in DRY_RUN.
    ...
    # PRE-TRAIN CACHE GATE: Report reuse eligibility before training.
    ...
    for seed in seeds:
        # Per-seed DataLoader: fresh Generator seeded from the run seed
        # ensures deterministic and reproducible sample ordering per seed.
        ...
        # Inject corrected execution lineage into run_config.lineage so the
        # corrected config_fingerprint is distinct from the historical,
        # invalidated Phase 46 config_fingerprint.  This does NOT mutate the
        # locked model hyperparameters or the locked scientific identity.
        # It only records the corrected execution provenance (FINAL_DEV
        # population + FINAL_SCALING-v1 SHAs + corrected implementation
        # version).
        ...
        # Validate cache:
        # 1. Must not be a historical invalidated run (0153/0154/0155).
        # 2. Must not be a corrected Phase 46 run whose historical predecessor matches
        #    the current seed's historical predecessor (to avoid reusing stale cache entries).
        ...
        # HARD-STOP PROBE: monkey-patch torch.optim.Optimizer.step (and all
        # subclass step methods) to raise a sentinel when --hard-stop-probe is
        # passed. This proves the official training path is wired correctly
        ...
        if "--hard-stop-probe" in sys.argv:
            ...
            # Walk all optimizer subclasses and patch their .step as well.
            # This handles Adam.step, AdamW.step, SGD.step, etc.
        ...
        # CORRECTED: evaluate_validation=False, validation_loader=None
        # This triggers FINAL_REFIT semantics (official_epoch = max_epochs)
        result = engine.train(
            run_id,
        ...
        # =========================================================================
        # Phase 46 STEP 10–12: per-seed strict-load + forward sanity + attention probe
        # =========================================================================
        param_count = sum(p.numel() for p in model.parameters())

        # State-dict schema fingerprint for cross-seed audit.
    ...
    # Build Phase 47 release with released=true (all gates passed)
    ...
    # =================================================================
    # O46.17–O46.32: Per-seed audit artifacts and cross-seed verification
    # =================================================================
