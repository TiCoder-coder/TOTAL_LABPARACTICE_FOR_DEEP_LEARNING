...
def emit_gradient_mutation_tests():
    ...
    # GC0 LOW_NORM (no clip call) → no mutation
    ...
    # GC0: no clipping call; simulate norm-only measurement
    _ = global_norm(m)
    ...
    # GC0 HIGH_NORM (no clip call) → no mutation
    ...
    # GC1 LOW_NORM (no rescale) → no mutation
    m = fresh_model(small=True)
    grads_before = {n: p.grad.clone() for n, p in m.named_parameters() if p.grad is not None}
    fp_before = fingerprint_gradients_dict(grads_before)
    pre_norm = global_norm(m)
    ...
    # GC1 HIGH_NORM (rescale) → mutation observed
    m = fresh_model(small=False)
    grads_before = {n: p.grad.clone() for n, p in m.named_parameters() if p.grad is not None}
    fp_before = fingerprint_gradients_dict(grads_before)
    pre_norm = global_norm(m)
...
def emit_nonfinite_guard_tests():
    ...
    # Detect NaN / +Inf / -Inf → must fail before optimizer.step()
    for cond in ['GC0', 'GC1']:
