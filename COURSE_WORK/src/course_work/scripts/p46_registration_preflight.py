...
def _build_corrected_run_config(seed: int, scaling_result: dict | None = None) -> dict:
    ...
    cfg = json.loads(json.dumps(locked))  # deep copy via JSON
    ...
    # Apply corrected lineage augmentation.
...
def main() -> int:
    ...
    # Step 1: Verify PHASE46_CORRECTIVE_RERUN is a canonical reason.
    ...
    # Step 2: Load real registry.
    registry = ExperimentRegistry(project_root=COURSE_WORK)
    records = registry._load_records()

    # Step 3: For each seed, verify duplicate detection and rerun eligibility.
    preflight = {
    ...
    for seed in seeds:
        # Two scenarios:
        #   A) corrected config (with lineage augmentation)
        #   B) raw locked config (no augmentation) — this is what would
        #      happen if the driver accidentally bypasses lineage injection.
        cfg_corrected = _build_corrected_run_config(seed)
        cfg_raw = json.loads(json.dumps(cfg_corrected))
        # Strip corrected lineage keys for raw config.
        for k in [
        ...
        # Test access check: target_access_mode is FINAL_DEV, not TEST.
        target_access_mode = cfg_corrected["data"].get("target_access_mode", "")
        test_access = target_access_mode == "TEST"

        # New run allocation: registry.allocate_run_id can be called without
        # collision since corrected fingerprint is distinct from historical.
        ...
        # Eligibility: PHASE46_CORRECTIVE_RERUN is in RERUN_REASONS, so any
        # duplicate (raw or augmented) is authorized.
        registration_eligible = canonical_reason_accepted and not test_access

        preflight["seeds"][str(seed)] = {
            # Raw scenario: would produce a duplicate fingerprint, requiring
            ...
            # Corrected scenario: lineage-augmented, distinct from historical.
            "corrected_scenario": {
    ...
    # Persist the report (non-official, just a preflight artifact).
