...
def main() -> int:
    ...
    # Build a minimal RunContext for fit_fold_local_scaler (no training).
    ctx = RunContext(
        project_root=PROJECT_ROOT,
        transformer_shortlist_path=PROJECT_ROOT / "artifacts/candidate_synthesis/transformer_candidate_shortlist.json",
        lstm_handoff_path=PROJECT_ROOT / "artifacts/lstm_tuning/phase44_rolling_origin_lstm_handoff.json",
        ...
        for f in folds:
            for stage in ["A", "B"]:
                try:
                    ...
                    # No outer-eval or Test IDs in fit population
                    # (audit stores count/fingerprint; bundle stores raw IDs)
