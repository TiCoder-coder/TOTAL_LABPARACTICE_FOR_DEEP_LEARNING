...
# Step 1: Patch env_mod.materialize_phase_1 BEFORE other modules use it.
...
def _patched_materialize_phase_1(root):
    ...
    if signoff_path.exists():
        # Skip the dependency freeze check; trust existing sign-off
...
def main():
    ...
    # Import run_single_condition AFTER patching (so its module-level
    # `from course_work.utils.environment import materialize_phase_1` captures
    # our patched version).
    from scripts import run_single_condition as rsc

    # Re-patch because the import may have overwritten our patch
    env_mod.materialize_phase_1 = _patched_materialize_phase_1
