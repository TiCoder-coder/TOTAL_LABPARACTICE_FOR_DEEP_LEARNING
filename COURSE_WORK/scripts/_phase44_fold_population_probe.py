...
def main() -> int:
    ...
    for c in candidates:
        ...
        fold_dataset_test_ids = set()  # compute via windowpop later

        for fold in folds:
            # Fit scalers per fold (Stage A and B)
            ...
            for role, ids, scaler in [
                ...
                # Sanity: target_id count must equal expected_sample_idx count
                ...
                # The MetricPopulationContext fingerprint MUST equal the
                # bundle fingerprint, which MUST equal the fold role
                # fingerprint. Use the SAME function everywhere:
                # derive_population_fingerprint(target_ids=...) — this
                # delegates to the same SHA256-of-sorted-target_id-strings
                # used by FoldDefinition.inner_train_fingerprint etc.
                expected_pop_fp = derive_population_fingerprint(
                    target_ids=list(ids),
                    split_id=role.upper(),
                )
                # Cross-check: also recompute via fold role fingerprint
                # to guarantee byte equality with the canonical fold value.
                fold_role_fp_map = {
                ...
                # Only consider positions that exist in this candidate's records
                ...
                # duplicates = observed count minus unique count
                dups = int(len(observed) - len(np.unique(observed)))
                # test leakage (sample_idx are positional — they cannot be test
                # rows directly, but the target_id of each observed sample_idx
                # MUST NOT be a Test ID). Map back via id_to_pos.
                test_leakage = sum(1 for t in observed_target_ids if t in test_set)

                # Validate MetricPopulationContext
                ctx_ok = True
                try:
                ...
                verdict = "PASS" if (
                    ...
                    and test_leakage == 0
                    and ctx_ok
                    and fp_agrees  # fold role fingerprint == derived fingerprint
                ) else "FAIL"
                if verdict == "FAIL":
                    fail_count += 1
    ...
    # Verify fingerprints are distinct across folds for inner_train (proves
    # RO1, RO2, RO3 are different)
