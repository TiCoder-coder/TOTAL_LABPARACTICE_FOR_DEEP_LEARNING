...
from course_work.data.scaling import load_validated_scaler_bundle  # noqa: E402
from course_work.models.revin import (  # noqa: E402
    AFFINE_BIAS_INIT,
    AFFINE_WEIGHT_INIT,
    REVIN_AFFINE,
    REVIN_EPS,
...
from course_work.utils.artifacts import canonical_json_bytes, read_json  # noqa: E402
...
def main():
    ...
    # --- Load RN0 reference and RN1 evidence ---
    ...
    # --- Strict BEST verification ---
    print("Running strict BEST verification...")
    strict = _load_strict_verification()
    if strict["status"] == "NOT_VERIFIABLE_ON_CURRENT_DEVICE":
        # Option A: recorded device unavailable on host. CPU fallback FORBIDDEN.
        # Emit a BLOCKED sign-off and exit; do NOT touch scientific state.
    ...
    # ============== O40.12–O40.18 Test-derived CSVs ==============
    ...
    # Recomputed by loading the BEST checkpoint and inspecting keys.
    ckpt = pd.DataFrame()
    ...
    # RN0 params from RN0 reference config
    rn0_param_count = 102209  # Phase 36 F256 reference, verified earlier
    ...
    # The RN1 training used canonical AdamW which covers all requires_grad parameters
    # of the wrapped model. Verified by checking parameter presence in model.parameters()
    # (the optimizer covered everything).
    ...
    # From Validation loader (we have full y_raw_wh); produce per-window means/stdevs over
    # the X-model feature set which we cannot recompute without re-loading windows. We use
    # the y_raw_wh distribution as a proxy target statistic.
    ...
    # ============== O40.34 Effect ==============
    delta_rmse = rn0_rmse - rn1_rmse  # positive if RN1 is better
    ...
    # ============== O40.38 Generalization diagnostics ==============
    ...
    # ============== O40.42 Phase 41 reference update ==============
    revin_config = {
        "eps": REVIN_EPS,
        "affine": REVIN_AFFINE,
        "affine_weight_init": AFFINE_WEIGHT_INIT,
    ...
    # Reuse the pre-existing discrepancies file (already contains H4 inherited warning).
    # Append empirical-comparison audit.
