...
# Canonical short -> long-form boundary protocol translation.
# The Phase 42 handoff stores the SHORT form (e.g. "WB0") but every layer that
# branches on this value (dataset suite, validation, metric, persistence) is
# written against the LONG form (e.g. "WB0_CONTEXT_CARRY_OVER"). Without this
# mapping the metric layer's `expected_sample_indices` quietly selects the
# WB1 column and reports 2924 expected IDs while the dataset yields 2960,
# causing the Validation pass to raise:
#   ValueError: "Observed sample population does not match the expected full split"
# This map is the single canonical translation point — every code path that
# passes boundary_protocol into the engine/registry must route through it.
_CANONICAL_BOUNDARY_PROTOCOL_MAP = {
    "WB0": "WB0_CONTEXT_CARRY_OVER",
    "WB1": "WB1_STRICT_ISOLATION",
}

# Mapping from raw Python exception class names to the canonical FailureType
# enum values accepted by `ExperimentRegistry.fail_run`. The exception handler
# must NEVER pass arbitrary class names like "ValueError" / "RuntimeError" to
# fail_run, because those are not members of FailureType and the registry
# raises `ValueError("Invalid failure type")`. The default for unrecognized
# exceptions is TRAINING_ERROR (the most general scientific-training category);
# numerical/runtime/population failures are routed to specific categories.
...
def _classify_failure(exc: BaseException) -> FailureType:
    ...
    # Default: scientific-training category is the safest for any unclassified
    # scientific-path error because the run never produced a COMPLETED state.
...
def _smoke_can_write_official() -> bool:
    ...
    return False  # explicit non-scientific marker
...
def prepare_phase43(project_root: Path) -> dict[str, Any]:
    ...
    # Snapshot prior PREP artifacts before re-writing
    snapshot = _snapshot_existing_prep_artifacts(project_root)

    # Tuning contract frozen
    ...
    # Stage lineage skeleton (no winners yet, status=PREPARED)
    lineage_rows = []
    ...
    # Run matrix skeleton (placeholder rows for prepared plan)
    run_matrix_rows = []
    for stage in planned_stages:
    ...
    # Phase 44 handoff (winner not yet known, ready_for_phase44=false)
...
def _resolve_reuse_or_fresh(
    ...
    """
    # Apply scientific contract enforcement BEFORE computing the fingerprint
    # so the fingerprint matches the canonical Phase 43 enforced config.
    from copy import deepcopy
    ...
    if reuse_record:
        # REUSE from active set — no registration, just read metrics
    ...
    # No reuse — fallback to normal duplicate policy
    ...
    if rerun_reason is not None and duplicate_records:
        # Check if any duplicate is COMPLETED
...
def run_scientific_training(project_root: Path, prepared: dict[str, Any]) -> dict[str, Any]:
    ...
    # Fresh DataLoader anchor for the shared contract
    set_seed(42)
    ...
    for stage_idx, planned in enumerate(planned_stages):
        ...
        for cand in planned.candidates:
            ...
            # Resolve REUSE vs FRESH — check active reuse set FIRST, then registry
            ...
            # No reuse available — register and train fresh
            fresh_total += 1
            ...
            # Fresh loaders per candidate to satisfy "fresh DataLoader"
            ...
            try:
                ...
                # The shared data contract carries the SHORT-form boundary_protocol
                # (e.g. "WB0") inherited from the Phase 42 handoff. The metric
                # layer (`expected_sample_indices`/`validate_population_coverage`)
                # and the dataset layer (`build_dataset_suite`/`SequenceWindowDataset`)
                # both rely on the LONG-form enum names ("WB0_CONTEXT_CARRY_OVER" /
                # "WB1_STRICT_ISOLATION"). Passing the short form through to
                # `engine.train()` silently routes `expected_sample_indices` onto the
                # `WB1_valid` column (because `"WB0" != "WB0_CONTEXT_CARRY_OVER"`),
                # producing 2924 expected IDs vs. the 2960 IDs the dataset actually
                # yields, and the Validation pass raises:
                #   ValueError: "Observed sample population does not match the
                #   expected full split"
                # Map short -> canonical long form here, exactly once, before any
                # call into the engine/registry/metric pipeline. This is the
                # single canonical translation point so we cannot regress to a
                # mixed short/long regime.
        ...
        # Build stage result for winner selection
        ranked = sorted(
            candidate_records,
            key=lambda r: (
    ...
    # Lifecycle-aware write: archive any PREPARED skeleton before writing the
    # final immutable winner. Shared write_bytes_once_or_verify immutability
    ...
    # Phase 44 handoff (lifecycle-aware final write — archives any PREPARED skeleton)
    phase44_payload = {
    ...
    if signoff_path.exists():
        ...
        # In rehearsal mode, prepare_phase43 may not have been run (or file was cleaned).
        # Create a minimal skeleton so the update logic works.
        signoff_payload = {
    ...
    # Phase 43 signoff (lifecycle-aware final write — archives any PREPARED skeleton)
    _write_phase43_final_json(
        project_root,
...
def _build_base_config(project_root: Path, reference) -> dict[str, Any]:
    if reference.phase20_exact_match and reference.reference_run_id:
        # Pull the existing Phase 20 config and clone it
        registry = ExperimentRegistry(project_root)
...
def _enforce_scientific_contract(cfg: dict[str, Any], contract: SharedDataContract, upstream_context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    # Window fingerprint for the contract lookback
    ...
    # NOTE: do NOT call _apply_reference_hyperparameters here. The base
    # config already has the reference hyperparameters baked in, and
    # StageExecutor mutates the per-factor value (e.g. model.hidden_size
    # for LT1) on top. Re-applying the reference values here would silently
    # clobber the candidate's distinguishing factor and collapse every
    # candidate in a stage into a single config_fingerprint. The candidate's
    # reference hyperparameters are already correct from the base.
    cfg.setdefault("data", {})
    ...
    # Registry validator requires the long-form enum; contract carries short
    # form from the handoff (e.g. WB0). Map short -> canonical long form.
...
def _archive_phase43_pre_rewrite(
    ...
    if archived_path.exists():
        # Append a counter to keep all archives intact
        i = 1
        while True:
    ...
    # Remove the original so the new write can proceed without verify-check
    path.unlink()
    return record
...
def _rehearse_production_path(project_root: Path) -> dict[str, Any]:
    ...
    REHEARSAL_EPOCHS = 2  # Very short for speed; contract/semantic correctness only
    ...
    # Copy reuse set from source.  Never call prepare_phase43(project_root)
    # here: that would write PREPARED skeletons into the active repository
    # before staging exists, downgrading a finalized PASS state.
    ...
    # Monkey-patch the contract's artifact root for rehearsal
    # (the prepared dict carries objects; we can't easily swap paths in
    # objects, so we intercept at the artifact-writer level by patching
    # write_text_once_or_verify and _write_json / _write_csv to use the
    # rehearsal directory for lstm_tuning paths)
    _rehearsal_active = {"enabled": True, "root": rehearsal_root}

    # We need to exercise the actual artifact writers against temp paths.
    # The cleanest way is to patch the project_root argument that gets passed
    # to every write_* function. Since these functions receive project_root as
    # a positional arg, we can use a simple approach: call the internal
    # orchestration with a modified project_root.
    # But this won't work for artifact writers that write outside lstm_tuning/.
    # Instead, we'll run the production path normally but intercept at the
    # orchestrator level. The simplest safe approach is to create a temporary
    # symlink or copy of the key directories.
    # Actually, let's use the cleanest approach: run the orchestration once with
    # a patched project_root that points to the temp dir, ensuring no active
    # artifacts are touched.

    # Create a temp project root that is a copy of the source project
    # Get reuse set and reference run id FIRST so we know what runs to copy
    ...
    # Strategy: create a fresh staging_root, but instead of using symlinks (which
    # cause .resolve() to follow back to source), create separate paths entirely.
    # We need:
    #   - Copy essential read-only dirs (src, scripts, data, configs, .venv symlink)
    #   - Symlink read-only artifact dirs (scaling, windows, splits, features, ...)
    #   - Fresh dirs for writeable dirs (lstm_tuning, experiments, runs)
    #   - Symlink the few run dirs we need from reuse set (without resolve following)
    staging_root = Path(tempfile.mkdtemp(prefix="phase43_staging_"))

    # Step 1: create fresh staging_root, copy essential dirs (no symlinks to project_root)
    # We avoid `.resolve()` roundtrips by NOT creating symlinks to paths in project_root.

    # Step 2: Make a "passthrough" helper that creates staging paths that look identical
    # to project_root paths for read-only access. We use os.symlink with absolute target
    # but mark it as non-resolving via separate path.
    #
    # SIMPLER APPROACH: Do not symlink anything. Instead, before running, temporarily
    # monkey-patch the project_root-aware lookups (resolve_lstm_t0_reference etc.)
    # but more importantly: write_function(path) calls use project_root. We must
    # intercept those.

    # Even simpler approach: REPLACE the symlink with a hard "pass-through" using
    # bind mount or just COPY. Let's COPY critical read-only directories (they're
    # not large enough to be a concern at this scale).

    # The run_registry_path in experiments is fixed: artifacts/experiments/experiment_registry.jsonl
    # Read-only artifact paths (Phase 9-13): COPY (not symlink) to avoid sandbox issues.
    # Note: "experiments", "lstm_tuning", and "runs" are handled separately below
    # (lstm_tuning is created fresh; experiments and runs are copied/symlinked separately).
    skip_subdirs = {
        "lstm_tuning",  # active Phase 43 artifact dir, fresh
        "experiments",  # experiment registry, copied separately
        "runs",         # run evidence, symlinked per-run
        "_history",     # historical archive, not needed for rehearsal
    }
    for subdir in sorted(p.name for p in (project_root / "artifacts").iterdir() if p.is_dir()):
        ...
        if src_p.exists() and not dst_p.exists():
            # Copy read-only content. We exclude large checkpoint files (*.pt) but
            # keep scalers/joblib files needed for checksum verification.
            shutil.copytree(
                src_p, dst_p, symlinks=True,
                ignore=shutil.ignore_patterns("__pycache__", "*.pyc", "*.pt", "*.pth"),
            )

    # Copy config files (must be writable copies — materialization writes there)
    ...
    # Copy source/script dirs (read-only for our purposes)
    ...
    # Symlink pyproject.toml, requirements.txt (top-level configs)
    ...
    # Now create FRESH (empty) dirs for writeable areas:
    staging_artifacts = staging_root / "artifacts"
    staging_artifacts.mkdir(parents=True, exist_ok=True)

    # lstm_tuning: fresh empty dir (artifact writers will populate)
    ...
    # experiments: copy the source registry (so it has historical data)
    ...
    # runs: create staging runs/ dir
    staging_runs_dir = staging_artifacts / "runs"
    staging_runs_dir.mkdir(parents=True, exist_ok=True)

    # For each needed run_id, symlink it from source to staging (read-only access).
    # New runs created during rehearsal will be regular dirs in staging.
    for run_id in needed_run_ids:
        ...
        if src_run.exists():
            # Symlink the entire run dir as read-only passthrough
    ...
    # Run prepare on the staging root (it will write PREPARED artifacts)
    staging_prepared = prepare_phase43(staging_root)

    # Clean PREPARED artifacts that production path needs to overwrite.
    # (prepare_phase43 writes skeleton files; run_scientific_training writes real content.)
    ...
    # Now run_scientific_training on staging root
    # BUT we need to make fresh candidates train for only REHEARSAL_EPOCHS
    # We patch the MAX_EPOCHS in the config by modifying _enforce_scientific_contract
    # to inject rehearsal epochs instead. Since we can't patch easily, we
    # temporarily patch the training contract.
    # Patch MAX_FRESH_SCIENTIFIC_RUNS for rehearsal to allow all fresh candidates
    # We patch the source module AND the imported name in this module.
    ...
    # Allow all fresh candidates in rehearsal (10 + buffer)
...
def _verify_rehearsal_outputs(
    ...
    # Verify LT1 winner exists and has correct structure
    ...
    # Count how many of O43.1-O43.37 are present
...
_PHASE43_EVIDENCE_STAGE_SOURCES: dict[str, list[tuple[str, Any, str]]] = {
    ...
    # The completed scientific attempt used its measured LH64 reference run
    # for the reference-valued option in LT2-LT5.  The aggregate recovery must
    # describe those physical runs as they exist; it must not synthesize a
...
def _lstm_parameter_count(config: dict[str, Any]) -> int:
    ...
    for _ in range(num_layers):
        # weight_ih + weight_hh + two bias vectors, once per direction.
        total += directions * (
...
def _build_final_winner_payload_from_evidence(
    ...
    """
    # Final stage winner = last non-skipped stage's winner
    ...
    # Reconstruct final tuned config from the winning run's stored config
...
def _finalize_phase43(project_root: Path) -> dict[str, Any]:
    ...
    # Reconstruct from the canonical registry plus physical per-run artifacts.
    # Aggregate Phase 43 files are deliberately not inputs: they are the state
    # being recovered and may contain PREPARED skeletons.
    ...
    # Write tuned winner (lifecycle-aware)
    ...
    # Write Phase44 handoff (lifecycle-aware)
    phase44_payload = {
        "lstm_tuning_version": "LSTM_TUNING-v1",
        "winner_run_id": final_run_id,
        "winner_config": final_cfg,
    ...
    # Build per-stage winners list for signoff
