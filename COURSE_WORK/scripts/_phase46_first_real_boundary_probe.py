...
def main() -> int:
    ...
    # 1. Snapshot canonical artifacts for comparison.
    canonical_artifacts = [
    ...
    # 2. Count current RUN_TR_FSD_* RUNNING records (must remain 0).
    import json
    ...
    # 3. Patch optimizer.step.
    original_step = _patch_optimizer_step()
    print("[Sentinel] torch.optim.Optimizer.step patched to raise boundary sentinel.")

    # 4. Run the driver subprocess; we expect a non-zero exit with our sentinel error.
    # We use --hard-stop-probe flag (the driver should recognize this).
    ...
    # Search for sentinel in stdout/stderr
    ...
    # 5. Check canonical artifacts unchanged.
    artifacts_changed = []
    ...
    # 6. Check no new RUN_TR_FSD_* RUNNING records.
    post_run_ids = []
    probe_run_ids = []
    if registry_path.exists():
        for line in registry_path.read_text().splitlines():
    ...
    # Clean up PROBE records created by this hard-stop probe.
    if probe_run_ids:
    ...
    # 7. Test access check
    test_accessed = False
    # Search for any test-access patterns in stdout
    test_patterns = ["evaluate_test", "test_dataset", "TestLoader", "test_loader"]
    for pat in test_patterns:
        if pat in combined.lower():
            # distinguish legitimate vs accidental access
            # Most usage of "test" is in preflight comments.
            # We check for actual Test path materialization.
