...
def _retry_after_failed_sandbox(project_root: Path, tmpdir: Path) -> dict[str, Any]:
    ...
    # Pick a fresh candidate mutation so this candidate is NOT a duplicate of
    # any historical Phase 43 record.
    cfg["model"]["hidden_size"] = 33  # impossible-neighbor (only 32,64,128 used)
    validated = validate_run_config(cfg, upstream)
    fingerprint = compute_config_fingerprint(validated)
    ...
    # === 1) Register the FAILED first attempt ===
    ...
    # === 2) Decide retry policy using resolve_duplicate_policy ===
    ...
    # === 3) Register the retry with the policy-derived rerun_reason ===
...
def main() -> int:
    ...
    try:
        ...
        # Confirm canonical registry remains untouched.
        canon = json.loads(
