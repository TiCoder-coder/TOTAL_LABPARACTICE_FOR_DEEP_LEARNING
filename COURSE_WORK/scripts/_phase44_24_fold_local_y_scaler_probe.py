...
def main() -> int:
    ...
    # Test IDs by definition do not appear in fold populations, but verify anyway.
    # The orchestrator's fold IDs are *train+val only* (no test); test leakage
    # would surface if any fit_target_id matches a Test ID.
    # Load the test population directly:
    test_df = pd.read_csv(
    ...
    for c in candidates:
        ...
        for fold in folds:
            for stage in ["A", "B"]:
                ...
                # Timestamps from window_records (only those matching fit_ids)
    ...
    # Provenance check: y_mean must NOT equal global y_mean exactly for fold-local
    ...
    # Distinct values: count unique (y_mean, y_std) across the 24 contracts
