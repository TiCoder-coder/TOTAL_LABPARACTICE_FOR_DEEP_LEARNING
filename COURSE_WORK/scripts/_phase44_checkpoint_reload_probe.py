...
def main() -> int:
    ...
    for run_id in STAGE_B_RUNS:
        ...
        try:
            ...
            # 1. Verify checkpoint exists (already done)
            # 2. Verify build_model_from_run_config works
            model = build_model_from_run_config(c.config)
            # 3. Strict state_dict load
            payload = torch.load(ckpt_path, map_location="cpu")
            model.load_state_dict(payload["model_state_dict"])
            # 4. Verify scaler/loader reconstruction via finalize helper
