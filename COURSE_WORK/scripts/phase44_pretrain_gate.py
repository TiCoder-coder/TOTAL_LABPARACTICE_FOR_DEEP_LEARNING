...
def check_fold_local_y_provenance() -> bool:
    ...
    for row in rows:
        # 1. Stage A Y scaler MUST come from inner_train only (no outer_eval, no test)
        if row["stage"] == "A":
            ...
            # Stage B Y scaler MUST come from outer_train (which may include inner_train+val+origin gap)
            # but MUST NOT use test.
            if row["test_rows_used"] != 0:
                fold_local_failures.append(
        ...
        # 2. Y mean/std must not equal global Phase9 exactly (proves NOT copied)
        #    (Values may be similar in practice — that's fine — but provenance
        #     proves they were separately fit. The 24-scaler probe runs the
        #     fold-specific fit and reports fit_min_ts/fit_max_ts. Provenance
        #     is the fit-count + ts window.)
        # We do NOT require literal numerical difference because the global
        # YS1 may happen to overlap; we require that fit_count > 0 and that
        # fit_min_ts/fit_max_ts spans a fold-specific range.
        if row["fit_count"] <= 0:
            fold_local_failures.append(
                f"{row['candidate']}/{row['fold']}/{row['stage']}: empty fit_count"
            )
        # The fit_min_ts and fit_max_ts together define the allowed training
        # window. They MUST be finite timestamps (not None).
        if row["fit_min_ts"] in (None, "n/a") or row["fit_max_ts"] in (None, "n/a"):
            fold_local_failures.append(
                f"{row['candidate']}/{row['fold']}/{row['stage']}: missing fit ts"
            )

    # Distinct (y_mean, y_std) counts is a strong indicator that values were
    # NOT copied from a single global source. We require ≥ 2 distinct values.
...
def check_fold_population_fingerprints() -> bool:
    ...
    # Verify distinct fingerprints per fold for each role
    ...
    # Verify inner_train ≠ inner_val ≠ outer_train ≠ outer_eval per fold
    by_fold_role = {}
    for r in rows:
...
def main() -> int:
    ...
    # 1-5. Run all probes via subprocess (zero official-training)
    ...
    # Run pytest (now includes the new fold-population contract tests
    # and the post-train recovery tests for --mode finalize)
    if not run_subcommand(
    ...
    print(f"")
    print('  cd "/Users/vientu/Deep Learning/TOTAL_LABPARACTICE_FOR_DEEP_LEARNING/COURSE_WORK"')
