...
def _training_attempt_guard() -> None:
    ...
    # Patch base + recursive subclasses
...
def _gate_scientific_evidence(
    ...
    # Materialize FINAL_DEV + FINAL_SCALING (read-only — these are
    # deterministic given the upstream contract; if already materialized,
    # this is a no-op).
    ...
    for seed in seeds:
        ...
        # Validate config_sha256 matches canonical
        ...
        # Validate population fingerprint
        ...
        # Validate checkpoint SHA matches
    ...
    # Cross-seed: same recipe, same config, same scalers
...
# ---------------------------------------------------------------------------
# Cross-seed consistency (for O46 audit writers)
# ---------------------------------------------------------------------------
...
def main() -> int:
    ...
    # Step 0: install the no-training guard
    ...
    # Step 2: gate the on-disk evidence
    try:
    ...
    # Step 3: cross-seed consistency — derive recipe_sha from official metadata
    # (each seed's metadata records the actual recipe_sha it was trained under).
    recipe_sha = records[0].get("recipe_sha256") or _compute_recipe_sha(SEEDS, epochs)
    lineage_sha = _compute_lineage_sha(locked_cfg)

    # Step 4: cross-seed consistency
    ...
    # Step 5: archive stale aggregate artifacts (idempotency)
    archive_targets = [
        "phase47_test_release.json",
        "phase_46_signoff.json",
        "three_seed_manifest.json",
    ...
    # Step 6: write phase47_test_release.json (FIX D-01: no set)
    excluded_list = [
    ...
    # Step 7: write three_seed_manifest.json
    manifest = {
        "phase": 46,
    ...
    # Step 8: write three_seed_contract.json
    contract = {
        "phase": 46,
        "version": "THREE_SEED_FINAL_RUNS-v1",
    ...
    # Step 9: write final_lock_verification.json
    ...
    # Step 10: write final_dev_population_manifest.json
    ...
    # Step 11: write three_seed_run_matrix.csv
    write_csv_safe(
    ...
    # Step 12: write three_seed_final_runs_summary.json
    ...
    # Step 13: write phase_46_signoff.json (FIX D-04: stricter gates)
    signoff_ok = (
        len(records) == 3
    ...
    # Step 14: write phase47_final_test_evaluation_handoff.json
    handoff_47 = {
    ...
    # Step 15: write O46 audit CSV/JSON artifacts
