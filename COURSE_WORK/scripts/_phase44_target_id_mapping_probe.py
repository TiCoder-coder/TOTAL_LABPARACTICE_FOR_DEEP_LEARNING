...
def main() -> int:
    ...
    # Focus on TR_C0_PRIMARY (L36) and TR_C2_ALT_LOOKBACK (L72)
    ...
    for cand in focus:
        ...
        # Build target_id -> records-row map
        ...
        for fold in folds:
            for role, ids in [
                ...
                # Use full IDs (the loader does its own target_id→position mapping)
                ...
                # Verify all kept_indices correspond to requested target_ids
                kept_target_ids = [
                ...
                # Spot check first 3 / last 3
                probe = (
                    ids[:3] if role in ("inner_train", "inner_val", "outer_eval") else []
                )
                if role == "inner_train":
                    probe = ids[:3] + ids[-3:]  # first 3 + last 3
                probe_kept = []
                for t in probe:
                    # Find the loader's kept position for this target
