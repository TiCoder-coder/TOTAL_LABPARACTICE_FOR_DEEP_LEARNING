...
def main() -> int:
    ...
    # Step 1: verify the OLD synthetic functions exist where we expect.
    ...
    # Step 2: monkey-patch them to RAISE if called.
    ...
    # Step 3: run the real orchestrator. If the synth mock were reachable,
    # this run would RAISE. It must complete normally.
    ...
    # Wrap the real functions to count invocations. The orchestrator imports
    # these names into its own namespace, so we must patch BOTH the source
    # module and the symbols re-imported into real_run.
    invocations = {
        "train_stage_a": 0,
    ...
    # Also patch the names re-imported into real_run's namespace
