...
def main() -> int:
    ...
    for cand_id in SANDBOX_CANDIDATES:
        ...
        try:
            ...
            # ── Fit Stage A and Stage B fold-local scalers ──
            ...
            # ── Stage A real-model YS1 path test ──
            ...
            for batch in inner_val_loader:
                x = batch["x"]
                y_model = batch["y_model"]  # scaled
                y_raw = batch["y_raw_wh"]  # raw Wh
            ...
            # ── NEW: Phase 44 fold-aware MetricPopulationContext smoke ──
            # This is the EXACT path that crashed in RUN_TR_ROB_0004 — compute
            # regression metrics on a fold subset using an explicit expected
            # population context (NOT the canonical full-split default).
            ...
            # PHASE 44 SINGLE-CANONICAL FINGERPRINT:
            # Use target_ids form so the fingerprint matches the fold-level
            # inner_val_fingerprint byte-for-byte.
            ...
            # ── Stage A inner_train (TRAIN mode) population-context metric ──
            ...
            # ── Stage B real-model YS1 path test ──
